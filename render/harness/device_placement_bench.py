...
"""Cycles CPU vs Metal device-placement sweep.
    ...
    python3 render/harness/device_placement_bench.py --self-test
    MERC_RENDER_DEVICE_PLACEMENT=1 python3 render/harness/device_placement_bench.py --write-evidence
...
ENTRY_REL = "render/metal/blender_entry.py"
...
def self_test() -> int:
    ...
    device_src = (ROOT / "render" / "metal" / "device.py").read_text(encoding="utf-8")
...
def measure(args: argparse.Namespace) -> dict[str, Any]:
    ...
    host = platform.node()
    work = Path(args.workdir) if args.workdir else ROOT / "render" / "corpus" / "output" / "placement"
    ...
    recommended = {
        ...
        "note": (
            ...
            "until these floors are copied into scripts/lib/device_placement.py "
    ...
    report = {
        ...
        "invocation": {
            ...
            "exclusion_proof": (
                ...
                f"{ENV_GATE}=1; listed in scripts/allowed-test-skips.txt; "
                "make test / make ci never set the env var"
            ),
            "command": (
                f"cd control && {ENV_GATE}=1 go test -count=1 -timeout 180m "
                "-run '^TestRenderDevicePlacementBench$' ."
            ),
            "blender_bin": blender,
            "blender_version": version,
            "script": ENTRY_REL,
            "harness": "render/harness/device_placement_bench.py",
            "engine": "CYCLES",
        ...
        "floors": recommended,
        "predicate": {
            "module": "scripts/lib/device_placement.py",
