...
class BackendRegistry:
    ...
    def capabilities(self) -> list[BackendCapability]:
        ...
        return [
            ...
            BackendCapability(
                name="blender-asset-preparation",
                version=blender.version or "unknown",
                revision="system",
                license="GPL-3.0-or-later executable; generated asset terms remain caller-owned",
                commercial_use=True,
                ...
                outputs=[
                    "candidate_blend",
                    "validated_input_glb_candidate",
                    "baked_png_textures",
                    "stage_receipts",
                ],
                hardware=["cpu", "gpu-optional"],
                quality_tier="bounded-production-preparation-candidate",
                precision=["float32"],
                input_limits={
                    "maximum_objects": 32,
                    "retopology_ratio": [0.05, 1.0],
                    "texture_resolution": [32, 4096],
                    "maximum_lods_per_object": 4,
                    "maximum_animation_frames": 10000,
                },
                operations=[
                    "retopology",
                    "uv_generation",
                ...
                input_modalities=[
                    "editable_blend",
                    "named_mesh_selection",
                    "explicit_material_and_animation_specification",
                ],
                output_coordinate_frame="preserves source Blender frame; GLB export is Y-up",
                scale_authority="inherits source scale without inventing metric authority",
                known_limitations=[
                    "Decimation is a retopology candidate, not hand-authored all-quad topology.",
                    "Character-lite rigging is bounded to a two-bone deformable mesh.",
                    "Texture baking proves execution and UV binding, not reference-image fidelity.",
                    "Every output remains a review candidate until domain acceptance passes.",
                ],
                confidence_semantics=(
                    "Stage receipts report exact Blender operations and structural facts; "
                    "perceptual and deformation quality are independently evaluated."
                ),
            ),
            BackendCapability(
                name="blender-appearance-authority",
                version=blender.version or "unknown",
                revision="system",
                license="GPL-3.0-or-later executable; generated renders remain caller-owned",
                commercial_use=True,
                redistribution="external executable",
                state=BackendState.AVAILABLE if blender.available else BackendState.UNAVAILABLE,
                outputs=[
                    "immutable_camera_render_receipts",
                    "material_structure_reports",
                    "lighting_structure_reports",
                    "exposure_brackets",
                    "pixel_comparisons",
                ],
                hardware=["cpu", "gpu-optional"],
                quality_tier="fixed-camera-appearance-acceptance",
                precision=["float32-render", "float64-camera-contract", "uint8-pixel"],
                input_limits={
                    "maximum_render_dimension": 4096,
                    "distortion_policy": "undistorted_input_or_explicit_applied_pass",
                    "material_parameter_tolerance": "fixed_by_benchmark_manifest",
                    "camera_nudging": False,
                },
                operations=[
                    "camera_intrinsics_extrinsics_validation",
                    "lens_distortion_governance",
                    "illumination_hypothesis_validation",
                    "pbr_material_validation",
                    "reflectance_highlight_validation",
                    "transparent_translucent_validation",
                    "fixed_camera_render_comparison",
                    "heldout_camera_validation",
                    "no_camera_nudge_acceptance",
                ],
                input_modalities=[
                    "editable_blend",
                    "immutable_camera_state",
                    "material_specification",
                    "lighting_hypothesis",
                    "reference_renders",
                ],
                output_coordinate_frame="preserves immutable Blender camera/world frame",
                scale_authority="inherits only from the supplied camera and scene authority",
                known_limitations=[
                    "A repeatability pass does not prove material inference from arbitrary photos.",
                    "Absent HDR evidence is represented as an explicit constant-world hypothesis.",
                    "Automated pixels complement rather than replace material-class review.",
                ],
                confidence_semantics=(
                    "Exact structure and camera identity are deterministic; pixel thresholds "
                    "are fixed per benchmark and inference confidence remains source-specific."
                ),
            ),
            BackendCapability(
