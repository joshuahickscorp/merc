...
// control/gateway_parity_harness.go (and control/gateway_parity_cli.go).
...
//
//	go run scripts/gateway-parity-v2.go -self-test-standin -out /tmp/parity-selftest.json
//
// Equivalent direct invocation:
//
//	cd control && go run . gateway-parity -self-test-standin -out …
package main
...
func locateControlDir() (string, error) {
	...
	if ok {
		cand := filepath.Join(filepath.Dir(thisFile), "..", "control")
	...
	return "", fmt.Errorf("cannot locate control/ (run from repo root or scripts/)")
}
