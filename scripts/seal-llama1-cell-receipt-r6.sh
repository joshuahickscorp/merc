...
payload = {
    ...
    "supersedes": {
        ...
        "reasons": [
            ...
            "r6 re-ran merc-agent bench-batch on this host under profile r9 with the current agent binary and sealed through scripts/write-bound-evidence.py",
        ],
    },
...
python3 "$ROOT/scripts/write-bound-evidence.py" \
  --out "$OUT_REL" \
  --harness "agent/src/main.rs:run_bench_batch (merc-agent 0.1.0 bench-batch, candle only, r6 settlement geometry)" \
