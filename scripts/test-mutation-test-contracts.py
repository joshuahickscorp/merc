...
CONTRACT_RESOLVER = ROOT / "scripts" / "mutation-test-contracts.py"
...
def mutation_sources() -> set[str]:
    ...
    output = subprocess.check_output(
        ["bash", "scripts/mutation-test.sh"], cwd=ROOT, env=env, text=True
    )
    sources: set[str] = set()
...
def listed_tests() -> set[str]:
    output = subprocess.check_output(
        ["go", "test", "-list", "^Test", "."], cwd=ROOT / "control", text=True
    )
