...
# scripts/derive-technical-exercises-receipt.py used by technical-exercises.sh.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
TMP="$(mktemp -d "${TMPDIR:-/tmp}/cx-tech-ex-fail.XXXXXX")"
cleanup() { rm -rf "$TMP"; }
trap cleanup EXIT
...
set +e
output="$(python3 "$ROOT/scripts/derive-technical-exercises-receipt.py" "$JSONL" "$EVIDENCE" 1 2>&1)"
status=$?
set -e
