...
validator=(
  python3 scripts/validate-agent-restart-receipt.py
  --run-id "$run_id"
  --commit "$commit"
  --image "$image"
...
MERC_CANARY_APPROVED_BUILD_HASHES='1111111111111111,2222222222222222' \
  bash scripts/go-closure-restart-storm.sh "$tmp/before.json" >/dev/null \
...
if MERC_TEST_DATABASE_URL="$test_url" \
  ...
  MERC_CANARY_APPROVED_AGENT_VERSIONS='1.2.3' \
  MERC_CANARY_APPROVED_BUILD_HASHES='1111111111111111,2222222222222222' \
    bash scripts/go-closure-restart-storm.sh "$tmp/one-unchanged.json" >/dev/null 2>&1; then
...
if MERC_TEST_DATABASE_URL="$test_url" \
  ...
  MERC_CANARY_APPROVED_BUILD_HASHES='1111111111111111,2222222222222222' \
    bash scripts/go-closure-restart-storm.sh "$tmp/before.json" >/dev/null 2>&1; then
...
"${doctor_env[@]}" \
  ...
  bash scripts/release-doctor.sh --check canary >/dev/null \
  || fail "release doctor rejected a complete canary authority input set"
if "${doctor_env[@]}" \
  bash scripts/release-doctor.sh --check canary >/dev/null 2>&1; then
