#!/usr/bin/env bash
# Syntax + contract check for the alpha-gate scaffold. Does not ssh, deploy,
# call Stripe, or start the soak.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd -P)"
# shellcheck source=scripts/alpha/lib.sh
. "$ROOT/scripts/alpha/lib.sh"

die() { printf 'validate-alpha: %s\n' "$*" >&2; exit 1; }
pass() { printf 'validate-alpha: PASS %s\n' "$*"; }

alpha_require_command jq
[ -f "$ROOT/docs/ALPHA_GATE_PLAN.md" ] || die "missing docs/ALPHA_GATE_PLAN.md"
[ -f "$ROOT/ops/go-no-go.json" ] || die "missing ops/go-no-go.json"

scripts=(
  scripts/alpha/lib.sh
  scripts/alpha/status.sh
  scripts/alpha/deploy.sh
  scripts/alpha/probes.sh
  scripts/alpha/stripe-test.sh
  scripts/alpha/offsite-restore.sh
  scripts/alpha/alert-sink.sh
  scripts/alpha/enrol-worker.sh
  scripts/alpha/canary-rehearsal.sh
  scripts/alpha/scenarios.sh
  scripts/alpha/soak.sh
  scripts/alpha/validate-scaffold.sh
)
for script in "${scripts[@]}"; do
  [ -f "$ROOT/$script" ] || die "missing $script"
  bash -n "$ROOT/$script" || die "bash -n failed for $script"
done
pass "bash syntax"

# Plan quotes every in-scope exit criterion.
for id in P1-STAGING P1-STRIPE-TEST P1-OFFSITE-RESTORE P1-ALERT-DELIVERY \
  P1-CANARY-REHEARSAL P1-RECOVERY-SOAK P1-GOVERNANCE; do
  grep -Fq "$id" "$ROOT/docs/ALPHA_GATE_PLAN.md" || die "plan omits $id"
  crit="$(jq -er --arg id "$id" '.open_p1[] | select(.id==$id) | .exit_criterion' "$ROOT/ops/go-no-go.json")"
  grep -Fq "$crit" "$ROOT/docs/ALPHA_GATE_PLAN.md" || die "plan does not quote $id exit_criterion"
done
grep -Fq 'P1-INDEPENDENT-APPROVAL' "$ROOT/docs/ALPHA_GATE_PLAN.md" \
  || die "plan does not record the dropped independent-approval gate"
pass "plan quotes in-scope exit criteria"

# Fail-closed contracts exist in the shared lib / soak driver.
for needle in alpha_reject_live_stripe alpha_require_boot alpha_require_prereqs \
  MERC_ALPHA_SOAK_I_AM_THE_SUPERVISOR VENDOR_WALL_UPPER_BOUND; do
  grep -Fq "$needle" "$ROOT/scripts/alpha/lib.sh" "$ROOT/scripts/alpha/soak.sh" \
    || die "scaffold missing fail-closed contract $needle"
done
grep -Fq 'sk_live_' "$ROOT/scripts/alpha/lib.sh" || die "live Stripe prefix guard missing"
grep -Fq '86400' "$ROOT/scripts/alpha/soak.sh" || die "soak missing 24h gate"
pass "fail-closed contracts"

# Two-device canary.
grep -Fq 'Mac Studio' "$ROOT/scripts/alpha/canary-rehearsal.sh" \
  || die "canary rehearsal does not name the Mac Studio"
grep -Fq 'MacBook' "$ROOT/scripts/alpha/canary-rehearsal.sh" \
  || die "canary rehearsal does not name the MacBook"
for adapter in kill_switch revocation intake_pause_result_retrieval no_payout_export; do
  grep -Fq "$adapter" "$ROOT/scripts/alpha/scenarios.sh" \
    || die "missing extra adapter $adapter"
done
pass "two-device canary + extra adapters"

# Deploy runner must not invoke ssh/rsync. Runbook text may mention ssh
# as documentation; go-closure uses `ssh --` / `rsync -` as real calls.
if grep -nE 'ssh -- |rsync -[a-zA-Z]' "$ROOT/scripts/alpha/deploy.sh"; then
  die "deploy.sh must not invoke ssh/rsync (runbook only)"
fi
pass "deploy runner does not ssh"

if command -v shellcheck >/dev/null 2>&1; then
  # shellcheck disable=SC2086
  shellcheck -x --severity=warning "${scripts[@]/#/$ROOT/}" || die "shellcheck failed"
  pass "shellcheck"
else
  printf 'validate-alpha: SKIP shellcheck (not installed)\n'
fi

pass "alpha-gate scaffold (this is not staging, Stripe, or soak evidence)"
