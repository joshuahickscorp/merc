...
#
# Wraps scripts/stripe-sandbox.sh. --check never opens a Stripe connection.
...
usage() {
  cat >&2 <<'USAGE'
usage: scripts/alpha/stripe-test.sh --print-runbook|--check|--execute

--check    classify keys locally, refuse live, require boot+staging (no Stripe I/O)
--execute  SUPERVISOR: scripts/stripe-sandbox.sh matrix against the deployed candidate
USAGE
  exit 2
}
...
execute_matrix() {
  ...
  [ -x "$ROOT/scripts/stripe-sandbox.sh" ] || alpha_die "missing scripts/stripe-sandbox.sh"
  local out
  out="$(mktemp "${TMPDIR:-/tmp}/merc-alpha-stripe.XXXXXX")"
  if ! bash "$ROOT/scripts/stripe-sandbox.sh" matrix >"$out" 2>"${out}.err"; then
