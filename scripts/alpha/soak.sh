...
usage() {
  cat >&2 <<'USAGE'
usage: scripts/alpha/soak.sh --print-start-command|--check|--execute [--duration N] [--interval N]
...
#    scripts/rollback.sh <prior-full-commit>
#    then redeploy the candidate (scripts/alpha/deploy.sh --print-runbook §4)
#    and re-run scripts/alpha/probes.sh --execute
