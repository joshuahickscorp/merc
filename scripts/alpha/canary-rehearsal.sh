...
#
# Wraps scripts/go-closure-canary-rehearsal.sh and scripts/alpha/scenarios.sh.
...
usage() {
  cat >&2 <<'USAGE'
usage: scripts/alpha/canary-rehearsal.sh --print-runbook|--check|--execute|--record-pass
USAGE
  exit 2
}
...
## Extra adapters (scripts/alpha/scenarios.sh) required by the exit criterion
#   kill_switch
#   revocation
...
#   MERC_CANARY_SCENARIO_DRIVER=$ROOT/scripts/canary-scenario-driver.sh
...
check_only() {
  ...
  [ -x "$ROOT/scripts/alpha/scenarios.sh" ] \
    || alpha_die "missing scripts/alpha/scenarios.sh"
  [ -x "$ROOT/scripts/alpha/enrol-worker.sh" ] \
    || alpha_die "missing scripts/alpha/enrol-worker.sh"
...
execute_rehearsal() {
  ...
  for scenario in $extras; do
    ...
    bash "$ROOT/scripts/alpha/scenarios.sh" run "$scenario" 1
  done
  if [ -n "${MERC_CANARY_SCENARIO_DRIVER:-}" ] && [ -x "${MERC_CANARY_SCENARIO_DRIVER}" ]; then
    ...
    bash "$ROOT/scripts/go-closure-canary-rehearsal.sh" \
