...
usage() {
  cat >&2 <<'USAGE'
usage: scripts/alpha/offsite-restore.sh --print-runbook|--check|--execute-restore|--record-pass
...
execute_restore() {
  ...
  [ -x "$ROOT/scripts/restore.sh" ] || alpha_die "missing scripts/restore.sh"
  local scratch
  scratch="merc_alpha_restore_$$"
  alpha_log "restoring latest offsite backup into scratch db $scratch (scripts/restore.sh)"
  if ! bash "$ROOT/scripts/restore.sh" --latest --db-only --to "$scratch"; then
