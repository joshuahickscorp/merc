...
#
#   scripts/alpha/deploy.sh --print-runbook
#   scripts/alpha/deploy.sh --check
#   scripts/alpha/deploy.sh --record-pass   # supervisor only, after probes PASS
set -euo pipefail
...
usage() {
  cat >&2 <<'USAGE'
usage: scripts/alpha/deploy.sh --print-runbook|--check|--record-pass
...
# - evidence/state/alpha-boot-green.json is BOUND PASS (VENDOR_WALL_UPPER_BOUND)
...
ssh $droplet 'cd /opt/merc && \\
  docker compose -f docker-compose.smallhost.yml up -d --no-deps --no-recreate postgres minio && \\
  docker compose -f docker-compose.smallhost.yml up -d --no-deps --build=false control caddy prometheus alertmanager'
...
#   MERC_BUILD_COMMIT=$commit docker compose -f docker-compose.smallhost.yml up -d --no-deps control caddy
#
...
check_local() {
  ...
  [ -f "$ROOT/pricing/board.json" ] || alpha_die "missing pricing/board.json"
  ...
  if grep -n 'sk_live_\|rk_live_\|pk_live_' "$ROOT/docker-compose.smallhost.yml" \
    "$ROOT/docker-compose.prod.yml" >/dev/null 2>&1; then
...
record_pass() {
  ...
  [ -f "$ALPHA_RECEIPT_DIR/P1-STAGING.probes.json" ] \
    || alpha_die "missing probe receipt $ALPHA_RECEIPT_DIR/P1-STAGING.probes.json; run scripts/alpha/probes.sh --execute first"
