...
#
# Usage:
#   scripts/seed-batch-infer-honeypot.sh
...
if [ -z "$AGENT_BIN" ]; then
  for candidate in \
    "$ROOT/agent/target/release/merc-agent" \
    "$ROOT/agent/target/debug/merc-agent" \
    "$(command -v merc-agent 2>/dev/null || true)"; do
    [ -n "$candidate" ] && [ -x "$candidate" ] && { AGENT_BIN="$candidate"; break; }
  done
fi
[ -n "$AGENT_BIN" ] && [ -x "$AGENT_BIN" ] \
  || die "merc-agent binary not found (set MERC_AGENT_BIN or build agent/target/release/merc-agent)"
...
# Take the exact bytes, never a re-serialized object: control/verification.go
...
echo "seed-batch-infer-honeypot: export these before 'cd control && go run . seed' to upload the input object:" >&2
