...
call elsewhere (e.g. energy-authority path bound, JSONL append still open).
    ...
    python3 scripts/test-evidence-writer-bypass.py --from-git HEAD --expect-fail
"""
...
ALLOWLIST = {
    ...
    "scripts/lib/write-bound-evidence.sh",
    "scripts/write-bound-evidence.py",
    "scripts/stamp-evidence-binding.py",
    "scripts/validate-evidence-binding.py",
    "scripts/test-evidence-writer-bypass.py",
    "control/receipt_identity.go",
    "control/receipt_identity_test.go",
    "control/evidence_binding_payload_guard_test.go",
}
...
def iter_scan_paths() -> list[Path]:
    ...
    for p in sorted(out, key=lambda x: x.as_posix()):
        ...
        if rel.startswith("scripts/test-") and rel not in {
...
def find_violations(rel: str, text: str) -> list[str]:
    ...
    # Fixture/unit tests under scripts/test-* write into $tmp, not the repo's
    # evidence/ tree. Still flag literal evidence/ destinations.
    is_test = rel.startswith("scripts/test-")
...
def main() -> int:
    ...
    if violations:
        ...
        print(
            "\nRoute each through scripts/write-bound-evidence.py, "
