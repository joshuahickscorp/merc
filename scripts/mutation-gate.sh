...
usage() {
  cat <<'EOF'
usage: bash scripts/mutation-gate.sh fast|authority|full|deep
...
full       all 109 mutations exactly once; hard budget 299 seconds by default
...
[ -f scripts/mutation-manifest.json ] || {
  echo "mutation gate requires scripts/mutation-manifest.json" >&2
  exit 2
}
...
manifest = json.loads(Path("scripts/mutation-manifest.json").read_text(encoding="utf-8"))
...
if mode in {"full", "deep"}:
    ...
    framework_changed = any(
        path.startswith("scripts/mutation-") or path in {"control/authority_callgraph_test.go", "control/money_authority_guard_test.go"}
        for path in changed
    )
    if override:
