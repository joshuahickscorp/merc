...
digests are pinned in agent/src/models.rs and recorded on each arm. Cosine
...
# Governed pins from agent/src/models.rs — refuse if on-disk digests diverge.
CANDLE_WEIGHTS = (
    Path.home()
...
def main() -> int:
    ...
    ap.add_argument(
        "--agent-bin",
        default=str(ROOT / "agent/target/release/merc-agent"),
    ...
    receipt.update(
        {
            ...
            "weights_comparability": {
                ...
                "note": (
                    ...
                    "agent/src/models.rs. Cosine gate is the outcome contract."
                ),
            ...
            "can_prove": [
                ...
                "named safetensors and F16 GGUF weight digests matching agent/src/models.rs pins",
    ...
    harness_rev = f"scripts/engine-parity-metal-embed-measure.py@{h_hash}"
    exact_config = (
