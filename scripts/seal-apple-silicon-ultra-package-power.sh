...
payload = {
    ...
    "harness": "scripts/seal-apple-silicon-ultra-package-power.sh (IOReport Energy Model package channels under merc-agent bench-batch load)",
...
python3 "$ROOT/scripts/write-bound-evidence.py" \
  --out "$OUT_REL" \
  --harness "scripts/seal-apple-silicon-ultra-package-power.sh (IOReport Energy Model package under merc-agent bench-batch)" \
