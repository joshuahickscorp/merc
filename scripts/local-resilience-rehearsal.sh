...
case "$MODE" in rollback|restart-storm|soak) ;; *)
  echo "usage: scripts/local-resilience-rehearsal.sh rollback|restart-storm|soak [--duration SECONDS] [--interval SECONDS]" >&2
  exit 2
esac
...
run_simulator() {
  (cd "$ROOT/control" && go run . release stripe-simulate --sequences 4096) > "$ART/payment-simulator.json"
