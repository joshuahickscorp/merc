...
#
# Prerequisites:
#   - agent/target/release/merc-agent (or AGENT_BIN)
...
raw["supersedes"] = {
    ...
    "reasons": [
        ...
        "r2 re-ran merc-agent bench-embed and sealed through scripts/write-bound-evidence.py",
    ],
}
...
python3 "$ROOT/scripts/write-bound-evidence.py" \
  --out "$OUT_REL" \
  --harness "agent/src/main.rs:run_bench_embed (merc-agent 0.1.0 bench-embed)" \
...
echo "sealed $OUT_REL (BOUND). Update control/evidence-manifest.json + runtime-authority.json if re-pointing authority."
