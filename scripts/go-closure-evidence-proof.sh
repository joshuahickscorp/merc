...
usage() {
  cat >&2 <<'USAGE'
usage: scripts/go-closure-evidence-proof.sh --target local|ssh \
...
host_prove() {
  ...
  python3 "$ROOT/scripts/validate-go-closure-evidence-chain.py" \
    --root "$GC_ROOT" \
...
gc_run_on_target "$target" scripts/go-closure-evidence-proof.sh \
