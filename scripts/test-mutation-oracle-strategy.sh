...
rg --fixed-strings 'full       all 84 mutations exactly once' scripts/mutation-gate.sh >/dev/null
...
[ "$list_count" = "84" ] || {
  echo "full alias did not list 84 mutations (got $list_count)" >&2
  exit 1
}
list_count_oracle="$(MERC_MUTATION_LIST=1 MERC_MUTATION_TEST_STRATEGY=oracle bash scripts/mutation-test.sh | wc -l | tr -d ' ')"
[ "$list_count_oracle" = "84" ] || {
  echo "oracle strategy did not list 84 mutations (got $list_count_oracle)" >&2
  exit 1
}
list_count_ws="$(MERC_MUTATION_LIST=1 MERC_MUTATION_TEST_STRATEGY=whole-suite bash scripts/mutation-test.sh | wc -l | tr -d ' ')"
[ "$list_count_ws" = "84" ] || {
  echo "whole-suite alias did not list 84 mutations (got $list_count_ws)" >&2
  exit 1
}

# ---------------------------------------------------------------------------
# Sandbox harness: tiny control package + one mutation case path.
# We do not run the real 84-case campaign. We exercise the oracle preflight and
