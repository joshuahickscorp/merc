...
rg --fixed-strings 'full       all 124 mutations exactly once' scripts/mutation-gate.sh >/dev/null
...
[ "$list_count" = "124" ] || {
  echo "full alias did not list 124 mutations (got $list_count)" >&2
  exit 1
}
list_count_oracle="$(MERC_MUTATION_LIST=1 MERC_MUTATION_TEST_STRATEGY=oracle bash scripts/mutation-test.sh | wc -l | tr -d ' ')"
[ "$list_count_oracle" = "124" ] || {
  echo "oracle strategy did not list 124 mutations (got $list_count_oracle)" >&2
  exit 1
}
list_count_ws="$(MERC_MUTATION_LIST=1 MERC_MUTATION_TEST_STRATEGY=whole-suite bash scripts/mutation-test.sh | wc -l | tr -d ' ')"
[ "$list_count_ws" = "124" ] || {
  echo "whole-suite alias did not list 124 mutations (got $list_count_ws)" >&2
  exit 1
}

# ---------------------------------------------------------------------------
# Sandbox harness: tiny control package + one mutation case path.
# We do not run the real 124-case campaign. We exercise the oracle preflight and
