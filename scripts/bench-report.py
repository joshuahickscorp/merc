...
those labels is not a claim anyone can check.
    ...
    python3 scripts/bench-report.py [--runs evidence/bench/runs.jsonl]
"""
...
# Economics, kept in step with control/pricing.go and pricing/board.json.
SUPPLIER_SHARE = 0.97
ELECTRICITY_USD_PER_KWH = 0.15
# Sustained package power. Measured when powermetrics is available; otherwise
# this is the documented apple_silicon_pro figure control/pricing.go uses, and
...
def main() -> int:
    ...
    path = Path(args.runs)
    if not path.exists():
        print(f"no runs at {path}; run scripts/bench-harness.py first", file=sys.stderr)
        return 2
    rows = load(path)
    if not rows:
