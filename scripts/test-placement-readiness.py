...
GATE = ROOT / "scripts" / "validate-placement-readiness.py"
...
def run_gate(cwd: Path, env: dict | None = None) -> tuple[int, str]:
    ...
    if env:
        full_env.update(env)
    proc = subprocess.run(
        [sys.executable, str(GATE if cwd == ROOT else cwd / "scripts" / "validate-placement-readiness.py"), "--json"],
        cwd=str(cwd),
        env=full_env,
        capture_output=True,
        text=True,
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="merc-placement-ready-") as tmp:
        ...
        auth_path = tmp_path / "control" / "runtime-authority.json"
