...
# .dockerignore excludes scripts/, and production mounts no web/ volume — pages
# come only from COPY web/ /web/ and COPY pricing/board.json in Dockerfile.control.
#
# Commit 0abb578a shipped an image whose web/ copy listed three pages and omitted
# pricing/board.json; the process could not start and nothing noticed. This gate
...
# Every file under web/ in the source tree must land at /web/... in the image.
# That is the claim COPY web/ /web/ makes; a selective COPY list is how pages
# disappeared before.
# Prefer `command find` so a shell wrapper (e.g. bfs shim) cannot break the gate.
while IFS= read -r -d '' src; do
  rel="${src#./}"
  REQUIRED_PATHS+=("/web/${rel}")
done < <(cd web && command find . -type f -print0)
