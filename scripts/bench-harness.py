...
Energy honesty:
  ...
  - Never estimate watts from a datasheet and present it as measurement.
    ...
    MERC_WRITE_ENERGY_AUTHORITY=1 python3 scripts/bench-harness.py --energy-authority \\
        --receipt evidence/perf/ioreport-gpu-energy-authority.json

    # package energy under sudo (powermetrics)
    sudo python3 scripts/bench-harness.py --sweep quick --power-backend powermetrics
...
# ASSUMED figure in control/pricing.go for apple_silicon_ultra. This lane does
...
def run_energy_authority(args: argparse.Namespace) -> int:
    ...
    payload = {
        ...
        "not_production_pricing_truth": (
            ...
            "be written into control/pricing.go without a separate pricing lane."
        ),
        "vs_assumed_sustained_watts": {
            ...
            "assumed_source": "control/pricing.go sustainedWattsByHWClass (not edited by this lane)",
