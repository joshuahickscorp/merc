...
usage() {
  echo "usage: scripts/go-closure-release-identity.sh --target local|ssh" >&2
  exit 2
}
...
# This copy contains only reviewed scripts/manifests and lets a first launch
# query the remote profile before it mutates a service.
gc_sync_bundle "$target"
gc_run_on_target "$target" scripts/go-closure-release-identity.sh
