...
payload = {
    ...
    "supersedes": {
        ...
        "reasons": [
            ...
            "r5 re-ran merc-agent bench-batch on this host under profile r9 and sealed through scripts/write-bound-evidence.py under the settlement geometry",
        ],
    },
...
python3 "$ROOT/scripts/write-bound-evidence.py" \
  --out "$OUT_REL" \
  --harness "agent/src/main.rs:run_bench_batch (merc-agent 0.1.0 bench-batch, candle only, r5 settlement geometry)" \
