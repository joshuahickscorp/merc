...
func TestNetworkFaultRegisterMatchesTheManifestAndTheSource(t *testing.T) {
	...
	raw, err := os.ReadFile(filepath.Join(root, "scripts", "network-fault-register.json"))
	if err != nil {
	...
	manifestRaw, err := os.ReadFile(filepath.Join(root, "scripts", "mutation-manifest.json"))
	if err != nil {
	...
	for _, entry := range reg.DeferredAbsent {
		...
		cmd := exec.Command("grep", "-rlE", pattern, filepath.Join(root, "control"),
			"--include=*.go")
		out, _ := cmd.Output()
		var live []string
...
// repoRootForTest resolves the repository root from the control/ working dir.
