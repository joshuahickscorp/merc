...
// and the binary; it did not copy pricing/board.json. The final stage is
...
func resolvePriceBoard(env string) (resolvedPriceBoard, error) {
	...
	// code path again.
	candidates := []string{"pricing/board.json", "../pricing/board.json"}
...
func releaseWebAsset(name string) string {
	for _, root := range []string{releaseWebRoot, "web"} {
