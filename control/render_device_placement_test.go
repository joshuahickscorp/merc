...
//	  python3 render/harness/device_placement_bench.py --write-evidence
//
//	MERC_RENDER_DEVICE_PLACEMENT=1 \
...
const (
	...
	devicePlacementHarnessRel = "render/harness/device_placement_bench.py"
	devicePlacementPredRel    = "scripts/lib/device_placement.py"
	devicePlacementEntryRel   = "render/metal/blender_entry.py"
	devicePlacementDeviceRel  = "render/metal/device.py"
)
...
func TestRenderDevicePlacementListedAsOptInSkip(t *testing.T) {
	...
	t.Fatal("TestRenderDevicePlacementBench must be listed in scripts/allowed-test-skips.txt so make ci does not treat the env gate as an unmarked skip")
}
