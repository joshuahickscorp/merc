...
//
// Writes evidence/perf/liveness-write-amplification.json when run from control/.
...
func TestLivenessWriteAmplificationBench(t *testing.T) {
	...
	report := writeAmpReport{
		...
		Invocation: writeAmpInvocation{
			...
			ExclusionProof:         "TestLivenessWriteAmplificationBench skips unless MERC_WRITE_AMP_BENCH=1; listed in scripts/allowed-test-skips.txt; make test / make ci never set the env var",
			Command:                "cd control && MERC_WRITE_AMP_BENCH=1 MERC_TEST_DATABASE_URL=postgres://cx:cx@localhost:5432/cx?sslmode=disable go test -count=1 -run '^TestLivenessWriteAmplificationBench$' -timeout 45m .",
