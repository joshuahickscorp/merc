...
// Recovery-lane observations are parsed by scripts/derive-recovery-receipts.py.
...
func requireRecoveryInfra(t *testing.T, containerEnv string) string {
	...
	if os.Getenv("MERC_RECOVERY_SUITE") != "1" {
		t.Skip("peer-container restart/partition tests run from scripts/recovery-suite.sh")
	}
