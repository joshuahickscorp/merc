...
func alphaHonestyRepoRoot(t *testing.T) string {
	...
	if _, err := os.Stat(filepath.Join(root, "scripts", "alpha", "lib.sh")); err != nil {
		t.Fatalf("repo root %s missing scripts/alpha/lib.sh: %v", root, err)
	}
	return root
}
...
func alphaLibOutput(t *testing.T, root string, extraEnv []string, snippet string) (string, error) {
	t.Helper()
	cmd := exec.Command("bash", "-c", "set -euo pipefail; . \""+root+"/scripts/alpha/lib.sh\"; "+snippet)
	cmd.Dir = root
...
func TestCitedCanaryComposeOverlayIsTracked(t *testing.T) {
	...
	runbook, err := os.ReadFile(filepath.Join(root, "scripts", "alpha", "deploy.sh"))
	if err != nil {
		t.Fatal(err)
	}
