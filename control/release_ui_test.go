...
func TestReleaseUIRejectsNonLoopbackListener(t *testing.T) {
	...
	for _, address := range []string{"127.0.0.1:0", "[::1]:0", "localhost:0"} {
		if err := releaseUIAddressAllowed(address); err != nil {
			t.Fatalf("loopback release UI address rejected %s: %v", address, err)
		}
	}
}
...
func TestReleaseUIOnlyReflectsWhitelistedState(t *testing.T) {
	root := t.TempDir()
	if err := os.MkdirAll(filepath.Join(root, ".merc-release"), 0o700); err != nil {
		t.Fatal(err)
	}
	state := `{"schema_version":2,"status":"planned","plan":{"plan_sha256":"plan"},"raw_secret":"super-secret-value"}`
	if err := os.WriteFile(releaseStatePath(root), []byte(state), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.WriteFile(releaseEvidencePath(root), []byte(evidence), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.MkdirAll(filepath.Join(root, "ops"), 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, "ops", "go-no-go.json"), []byte(`{"readiness_score":83,"decisions":{"supervised_stripe_test_mode_private_canary":"NO_GO"},"open_p1":[{"id":"P1-STAGING","owner":"operations","blocker":"super-secret-value"}]}`), 0o600); err != nil {
		t.Fatal(err)
	}
