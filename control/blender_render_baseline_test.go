...
//
// Writes evidence/perf/blender-render-baseline.json when run from control/.
//
...
const (
	...
	blenderRenderScriptRel        = "scripts/blender-render-baseline.py"
...
func TestBlenderRenderBaseline(t *testing.T) {
	...
	command := "cd control && MERC_BLENDER_RENDER_BENCH=1 go test -count=1 -run '^TestBlenderRenderBaseline$' -timeout 45m ."
	report := blenderRenderReport{
		...
		Invocation: blenderRenderInvocation{
			...
			ExclusionProof:         "TestBlenderRenderBaseline skips unless MERC_BLENDER_RENDER_BENCH=1; listed in scripts/allowed-test-skips.txt; make test / make ci never set the env var",
			Command:                command,
			BlenderBin:             blender,
...
func TestBlenderRenderBaselineListedAsOptInSkip(t *testing.T) {
	raw, err := os.ReadFile(filepath.Join("..", "scripts", "allowed-test-skips.txt"))
	if err != nil {
	...
	t.Fatal("TestBlenderRenderBaseline must be listed in scripts/allowed-test-skips.txt so make ci does not treat the env gate as an unmarked skip")
}
