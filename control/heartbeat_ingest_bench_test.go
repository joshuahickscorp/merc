...
//	  bash -c 'cd control && go test -count=1 -run '^TestHeartbeatIngestBench$' -timeout 2h .'
//
...
func TestHeartbeatIngestBench(t *testing.T) {
	...
	report := heartbeatIngestReport{
		...
		Invocation: heartbeatIngestInvocation{
			...
			Command:                "bash scripts/with-isolated-test-db.sh bash -c 'cd control && go test -count=1 -run ^TestHeartbeatIngestBench$ -timeout 2h .'",
			ExcludedFromNormalGate: true,
			ExclusionProof:         "TestHeartbeatIngestBench skips unless MERC_HEARTBEAT_INGEST_BENCH=1; listed in scripts/allowed-test-skips.txt; make test / make ci never set the env var",
			Mode:                   mode,
