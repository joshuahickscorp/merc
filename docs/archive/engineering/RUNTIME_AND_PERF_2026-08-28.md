...
> in `docs/RUNTIME_AND_PERF.md § "Merc efficiency frontier"` is the same chain. Use `docs/archive/engineering/PROGRAMME.md § "Merc shippability status"`
...
> `docs/archive/engineering/PROGRAMME.md § "Path to 10 — what an agent can finish alone, and what only you can do"`.)
...
`docs/archive/engineering/PROGRAMME.md § "Which lane can actually pay a supplier"` computes the supplier break-even. Restating it against
...
> multiples. Use `docs/archive/engineering/PROGRAMME.md § "Merc shippability status"` and bound receipts under
> `evidence/` for current claims. This file is retained as an audit trail, not
> an operator performance reference. (Banner pattern matches `docs/archive/engineering/PROGRAMME.md § "Path to 10 — what an agent can finish alone, and what only you can do"`.)
...
The direct adversarial work here is marked `NOT_EXTERNALLY_INDEPENDENT`: the
...
No acceptance condition in this tranche depended on external-review evidence. The
...
| Level-B / go-closure canary design | **2** distinct approved workers | `docs/archive/engineering/PROGRAMME.md § "Level-B operator handoff"`, `ops/go-closure-inputs.json` ("two distinct operator-controlled Metal worker identities") — still a thin book, not a marketplace. |
