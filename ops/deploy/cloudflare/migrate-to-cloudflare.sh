...
#
#   ops/deploy/cloudflare/migrate-to-cloudflare.sh <postgres-url>
#
...
echo "== 2. apply schema"
psql "$PG" -v ON_ERROR_STOP=1 -q -f "$ROOT/src/control/schema.sql" >/dev/null
echo "   schema applied"
...
echo "   then put the returned id in ops/deploy/cloudflare/wrangler.jsonc"

echo "== 5. deploy the control plane (needs Workers Paid for Containers)"
echo "   cd ops/deploy/cloudflare && wrangler deploy"
