...
MERC_RESTORE_DRILL_EVIDENCE="$ROOT/evidence/autonomous/restore-drill.json" \
  ...
  bash "$ROOT/ops/scripts/restore-drill.sh"
restore_exit=$?
set -e
...
set +e
bash "$ROOT/ops/scripts/local-independent-restore.sh"
independent_exit=$?
set -e
...
if command -v age >/dev/null 2>&1 && command -v age-keygen >/dev/null 2>&1; then
  ...
  if bash "$ROOT/ops/scripts/test-backup-envelope.sh"; then
    envelope_ok=true
  fi
else
...
env -u MERC_RECOVERY_SUITE -u MERC_RECOVERY_PG_CONTAINER -u MERC_RECOVERY_MINIO_CONTAINER \
  ...
  bash "$ROOT/ops/scripts/test-backup-age-metric.sh"
age_metric_exit=$?
set -e
log "backup-age-metric exit=$age_metric_exit"

suite_exit=0
python3 "$ROOT/ops/scripts/derive-recovery-receipts.py" \
