...
(cd src/control && go build -o "$ART/merc" .)
...
psql "$TEST_DATABASE_URL" -v ON_ERROR_STOP=1 --single-transaction -f src/control/schema.sql >/dev/null
...
(cd src/control && go vet ./... && go test -json ./... | tee "$ART/go-test.json")
if [ "$USE_DOCKER" = "1" ]; then
...
if [ "$SKIP_LIVE" != "1" ]; then
  cargo build --release --manifest-path src/agent/Cargo.toml
  ...
  MERC_AGENT_BIN="$AGENT_BIN" MERC_MODEL_CACHE="$MODEL_CACHE" \
    ...
    ops/scripts/seed-batch-infer-honeypot.sh >"$ART/batch-infer-honeypot.json" 2>"$ART/batch-infer-honeypot.log"
