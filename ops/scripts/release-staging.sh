...
case "$MODE" in
  render)
    ...
    ;;
  validate)
    bash "$ROOT/ops/scripts/validate-go-closure-scaffold.sh" >/dev/null
    jq -e '
    ...
    for required in ops/scripts/backup.sh ops/scripts/go-closure-rollback-rehearsal.sh ops/monitoring/alertmanager.yml; do
    ...
    python3 - "$ROOT" "$payload" "ops/scripts/release-staging.sh" <<'PY'
import json, sys
...
PY
    ...
    ;;
  *) echo 'usage: ops/scripts/release-staging.sh render|validate' >&2; exit 2 ;;
esac
