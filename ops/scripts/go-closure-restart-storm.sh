...
usage() {
  echo "usage: ops/scripts/go-closure-restart-storm.sh --target local|ssh --check|--execute" >&2
  exit 2
}
...
validate_agent_receipt() {
  ...
  python3 "$ROOT/ops/scripts/validate-agent-restart-receipt.py" "$file" \
...
host_storm() {
  ...
  merc_emit_bound_json "$agent_receipt" "ops/scripts/go-closure-restart-storm.sh" "$payload" \
...
gc_run_on_target "$target" ops/scripts/go-closure-restart-storm.sh "$operation"
