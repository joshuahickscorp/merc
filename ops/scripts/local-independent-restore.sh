...
docker exec -i "$A_PG" psql -X -v ON_ERROR_STOP=1 -U cx -d cx < "$ROOT/src/control/schema.sql" > /dev/null 2> "$WORK/schema.log"
...
merc_emit_bound_json "$receipt" "ops/scripts/local-independent-restore.sh" "$payload" \
...
python3 - "$ROOT" "$receipt" "ops/scripts/local-independent-restore.sh" <<'PY'
import json, sys
