...
Derived artifacts only. This script reads the real lockfiles / manifests:
  ...
  src/control/go.mod, src/control/go.sum          (git show if not on disk)
  src/agent/Cargo.lock + crate Cargo.toml     (declared license fields)
...
# (not distributed as source) + Apache-2.0 src/agent/clients (distributed to
...
def main() -> int:
    ...
    inputs = {
        "src/control/go.mod": git_show("src/control/go.mod"),
        "src/control/go.sum": git_show("src/control/go.sum"),
        "src/agent/Cargo.lock": git_show("src/agent/Cargo.lock"),
        "src/agent/Cargo.toml": git_show("src/agent/Cargo.toml"),
    ...
    go_mod_rows = parse_go_mod(inputs["src/control/go.mod"])
    go_sum_rows = parse_go_sum(inputs["src/control/go.sum"])
    ...
    cargo_packages = parse_cargo_lock(inputs["src/agent/Cargo.lock"])
            ...
            rust_components.append(
                {
                    ...
                    "license_source": "src/agent/Cargo.toml license = Apache-2.0",
                    "compatibility": {
                        "verdict": "FIRST_PARTY",
    ...
    doc = {
        ...
        "generator": "ops/scripts/generate-license-inventory.py",
        "honesty": {
            ...
            "go_licenses": (
                ...
                "proxy.golang.org for every module listed in src/control/go.mod. "
                "Go module metadata has no license field; nothing is guessed."
            ),
            "cargo_licenses": (
                "Declared license fields from the crate Cargo.toml matching "
                "src/agent/Cargo.lock name+version in the local cargo registry."
            ),
            "excluded": (
...
def render_inventory_md(doc: dict[str, Any]) -> str:
    lines = [
        ...
        "`ops/scripts/generate-license-inventory.py`. Do not hand-edit. "
    ...
    lines += [
        ...
        "## Counts",
        "",
        f"- Go modules in `src/control/go.mod`: {counts['go_mod_modules']}",
        ...
        f"- Cargo packages in `src/agent/Cargo.lock`: {counts['cargo_packages']}",
