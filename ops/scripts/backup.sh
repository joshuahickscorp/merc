...
for arg in "$@"; do
  case "$arg" in
    ...
    --dry-run) DRY_RUN=1 ;;
    *)
      echo "usage: ops/scripts/backup.sh [--db-only] [--dry-run]" >&2
      exit 2
      ;;
  esac
done
...
if [ -n "$RESULT_FILE" ]; then
  ...
  case "$RESULT_FILE" in
    */evidence/*|"$ROOT"/evidence/*)
      ...
      merc_emit_bound_json "$RESULT_FILE" "ops/scripts/backup.sh" "$result_tmp" \
...
log "restore: ops/scripts/restore.sh $TS    (or --latest) · see docs/RUNBOOKS.md"
