...
ALLOWLIST="ops/scripts/allowed-test-skips.txt"
...
if [ -z "$LOG" ]; then
  ...
  LOG="$(mktemp)"
  trap 'rm -f "$LOG"' EXIT
  if ! (cd src/control && MERC_TEST_DATABASE_URL="${MERC_TEST_DATABASE_URL:-}" \
          bash ../../ops/scripts/with-isolated-test-storage.sh \
