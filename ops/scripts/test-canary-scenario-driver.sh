#!/usr/bin/env bash
# Exercise ops/scripts/canary-scenario-driver.sh against a locally-running control
...
DRIVER="$(cd ops/scripts && pwd -P)/canary-scenario-driver.sh"
[ -x "$DRIVER" ] || fail "ops/scripts/canary-scenario-driver.sh is not executable"
...
validate_receipt() {
  ...
  python3 ops/scripts/validate-canary-scenario-receipt.py "$file" \
...
# ---------------------------------------------------------------------------
{
  ...
  if grep -n '00000000-0000-0000-0000-0000000000b[12]' src/control/seed.go \
    | grep -v '//' >/dev/null 2>&1; then
    # Only fail if those appear as the demoWorkerID constants (not comments).
    if grep -E 'demoWorkerID2?\s*=\s*"00000000-0000-0000-0000-0000000000b' src/control/seed.go >/dev/null; then
      fail "seed demo workers are still version-nibble 0 UUIDs"
    fi
  fi
  grep -E 'demoWorkerID\s*=\s*"00000000-0000-4000-8000-0000000000b1"' src/control/seed.go >/dev/null \
  ...
  # behaviour until ops/scripts/seed-batch-infer-honeypot.sh has run.
  grep -q 'honeypots/batch_infer' src/control/seed.go \
    || fail "seed.go does not install a batch_infer honeypot input"
  if grep -qiE 'unmeasured' src/control/seed.go; then
    fail "seed.go still writes an unmeasured batch_infer known answer"
  fi
  grep -q 'MERC_BATCH_INFER_HONEYPOT_ANSWER' src/control/seed.go \
    || fail "seed.go does not accept a measured batch_infer answer"
  [ -x ops/scripts/seed-batch-infer-honeypot.sh ] \
    || fail "ops/scripts/seed-batch-infer-honeypot.sh missing or not executable"
  ...
  grep -q 'known_answer_utf8' ops/scripts/seed-batch-infer-honeypot.sh \
    || fail "seed-batch-infer-honeypot.sh does not take the exact answer bytes"
  grep -q 'seed-batch-infer-honeypot.sh' ops/scripts/prove-local.sh \
...
JSON
  set +e
  python3 ops/scripts/validate-canary-scenario-receipt.py "$fake" \
