...
via the single write path.
    ...
    python3 ops/scripts/stamp-evidence-binding.py
    python3 ops/scripts/stamp-evidence-binding.py --dry-run
    python3 ops/scripts/stamp-evidence-binding.py --no-census
"""
...
# Keep in lockstep with ops/scripts/validate-evidence-binding.py. These trees are
...
def write_census(counts: dict[str, int], artifacts: list[dict], dry_run: bool) -> None:
    ...
    identity = default_bound_identity(
        ROOT,
        harness_revision="ops/scripts/stamp-evidence-binding.py",
