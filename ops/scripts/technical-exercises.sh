...
if [ -z "${S3_ENDPOINT:-}" ]; then
  exec bash "$ROOT/ops/scripts/with-isolated-test-storage.sh" bash "$ROOT/ops/scripts/technical-exercises.sh" "$@"
fi
...
python3 "$ROOT/ops/scripts/validate-authorization-matrix.py" >/dev/null
...
python3 "$ROOT/ops/scripts/derive-technical-exercises-receipt.py" \
