...
rg -q 'ops/scripts/lib/stripe-sandbox-contract\.sh' ops/scripts/merc-credentials.sh \
  || die "credential checker does not consume the Stripe sandbox authority"
if rg -q 'settles in USD|NO usd bucket|usd is enabled' ops/scripts/merc-credentials.sh; then
...
if rg -n 'currency=usd|country == "US"|US test account' \
  "$ROOT/ops/scripts/stripe-sandbox.sh" "$ROOT/ops/scripts/stripe-sandbox-scenarios.sh" \
  >/dev/null; then
  die "formal Stripe matrix still contains the obsolete USD/US authority"
fi
for script in ops/scripts/stripe-sandbox.sh ops/scripts/stripe-sandbox-scenarios.sh; do
  rg -q 'ops/scripts/lib/stripe-sandbox-contract[.]sh' "$ROOT/$script" \
...
rg -q 'ops/scripts/lib/stripe-sandbox-contract[.]sh' "$ROOT/ops/scripts/stripe-sandbox-nonconnect.sh" \
  || die "nonconnect driver does not source the shared candidate authority"
rg -q 'ops/scripts/lib/stripe-sandbox-nonconnect.py' "$ROOT/ops/scripts/stripe-sandbox-nonconnect.sh" \
  || die "nonconnect wrapper does not invoke the python driver"
rg -q 'check|matrix|nonconnect' "$ROOT/ops/scripts/stripe-sandbox.sh" \
...
for required in \
  ...
  rg -Fq -- "$required" "$ROOT/ops/scripts/lib/stripe-sandbox-nonconnect.py" \
    || die "nonconnect driver does not name scenario: $required"
done
if rg -q 'merc-secrets[.]env' "$ROOT/ops/scripts/stripe-sandbox-nonconnect.sh" \
  && ! rg -q 'refusing to read .merc-secrets.env' "$ROOT/ops/scripts/stripe-sandbox-nonconnect.sh"; then
...
for required in \
  ...
  rg -Fq -- "$required" "$ROOT/ops/scripts/stripe-sandbox-scenarios.sh" \
    || die "scenario driver does not prove webhook outcome: $required"
done
if rg -Fq "evidence[submit]" "$ROOT/ops/scripts/stripe-sandbox-scenarios.sh"; then
  die "scenario driver nests Stripe dispute submit under evidence"
fi
rg -q '\[ "\$live_alias_present" = false \]' "$ROOT/ops/scripts/release-doctor.sh" \
  || die "release doctor can report Stripe ready while a live alias is present"
rg -q 'merc_stripe_distinct_endpoint_ids' "$ROOT/ops/scripts/release-doctor.sh" \
  || die "release doctor does not reject aliased endpoint IDs"
rg -q 'merc_stripe_valid_staging_hostname' "$ROOT/ops/scripts/release-doctor.sh" \
...
run_rejected_preflight() {
  ...
  output="$(env \
    ...
    "$ROOT/ops/scripts/stripe-sandbox.sh" check)"
  status=$?
  set -e
