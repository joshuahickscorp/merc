#!/usr/bin/env python3
"""Fail closed when a Python or TypeScript SDK path is not registered in src/control/api.go.
...
def registered_routes() -> set[str]:
    ...
    if not routes:
        fail("src/control/api.go has no mux.Handle registrations")
    if len(routes) != len(set(routes)):
        fail("src/control/api.go registers a duplicate method/path")
    return set(routes)
...
def normalize_sdk_path(path: str) -> str:
    ...
    # used by src/control/api.go ({id}, {path...}, {name}, ...).
...
def main() -> None:
    ...
    if missing:
        fail(
            "SDK path(s) not registered in src/control/api.go:\n  - "
            + "\n  - ".join(missing)
        )
