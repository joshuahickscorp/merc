...
def write_stage(name: str, doc: dict[str, Any]) -> Path:
    ...
    identity = default_bound_identity(
        ROOT,
        harness_revision="ops/scripts/alpha/close-staging-loop.py",
...
def main() -> int:
    ...
    quote_receipt = {
        ...
        "honeypot_frac": 0.0,
        "source_gate": (
            "POST /v1/quote is mounted behind Server.authBuyer (src/control/api.go:170,360-376). "
            "Missing bearer is HTTP 401 'missing or malformed Authorization bearer token'. "
            "After auth, validateCurrentUniformCanaryAuthority (src/control/task_economic_authority.go:29-35) "
    ...
    submit_receipt = {
        ...
        "billing": billing,
        "source_gate": (
            "POST /v1/jobs is mounted behind Server.authBuyer (src/control/api.go:136). "
            "Funding is POST /v1/billing/topup (src/control/api.go:175); the operator buyer "
    ...
    claim_receipt = {
        ...
        "source_gate": (
            ...
            "(src/control/buyer_supplier_independence.go:176). EnsureSupplierForBuyer "
    ...
    write_stage(
        "execute",
        {
            ...
            "executor": "inline well-formed infer artifact (emit-infer-artifact is not in this tree's src/agent/)",
    ...
    write_stage(
        "verification-accept",
        {
            ...
            "source_gate": (
                ...
                "(src/control/verification.go). Same-supplier redundancy records "
                "redundancy_same_supplier / NO_INDEPENDENT_SUPPLIER and fails closed "
                "(src/control/verification.go:206-214)."
            ),
            "remaining_refusal": None
            if accept_ok
            else {
    ...
    summary = {
        ...
        if quote_ok
            ...
            "source": "src/control/api.go:170 mounts POST /v1/quote behind authBuyer; api.go:364-366 writes 401 missing or malformed Authorization bearer token",
