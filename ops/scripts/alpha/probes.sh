...
#
#   ops/scripts/alpha/probes.sh --print-runbook
#   ops/scripts/alpha/probes.sh --check
#   ops/scripts/alpha/probes.sh --execute     # HTTPS only; no ssh
set -euo pipefail
...
usage() {
  cat >&2 <<'USAGE'
usage: ops/scripts/alpha/probes.sh --print-runbook|--check|--execute
USAGE
  exit 2
}
