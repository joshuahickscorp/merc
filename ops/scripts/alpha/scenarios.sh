...
#
# Known go-closure scenarios are delegated to ops/scripts/canary-scenario-driver.sh.
# Usage: ops/scripts/alpha/scenarios.sh run <scenario> <minimum>
set -euo pipefail
...
delegate() {
  ...
  exec bash "$ROOT/ops/scripts/canary-scenario-driver.sh" run "$scenario" "$minimum"
}
