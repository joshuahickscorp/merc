...
#
#   ops/scripts/alpha/catalogue-board.sh --diagnose
#   ops/scripts/alpha/catalogue-board.sh --print-runbook
#
...
usage() {
  cat >&2 <<'USAGE'
usage: ops/scripts/alpha/catalogue-board.sh --diagnose|--print-runbook
USAGE
  exit 2
}
...
# loadCurrentPublicCatalogue (src/control/api.go). The 503 string is
...
#    back to the current document (src/control/pricing_publication.go +
...
diagnose() {
  ...
  if [ -x "$ROOT/ops/scripts/alpha/remote.py" ]; then
    python3 "$ROOT/ops/scripts/alpha/remote.py" \
