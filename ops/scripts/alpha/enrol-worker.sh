...
#
#   ops/scripts/alpha/enrol-worker.sh --device studio|laptop --print-runbook
#   ops/scripts/alpha/enrol-worker.sh --device studio --check
set -euo pipefail
...
usage() {
  cat >&2 <<'USAGE'
usage: ops/scripts/alpha/enrol-worker.sh --device studio|laptop --print-runbook|--check
USAGE
  exit 2
}
