...
#
# Prerequisites:
#   - src/agent/target/release/merc-agent (or AGENT_BIN) built with metal AFTER
#     src/control/runtime-authority.json points this cell at r7.json (the authority
...
payload = {
    ...
    "supersedes": {
        ...
        "reasons": [
            ...
            "r7 re-ran merc-agent bench-batch on this host under profile r9 with the current agent binary (authority frozen at the r7 path before rebuild) and sealed through ops/scripts/write-bound-evidence.py",
        ],
    },
...
python3 "$ROOT/ops/scripts/write-bound-evidence.py" \
  --out "$OUT_REL" \
  --harness "src/agent/src/main.rs:run_bench_batch (merc-agent 0.1.0 bench-batch, candle only, r7 settlement geometry)" \
