...
const pageNames = ['clients/web/index.html', 'clients/web/buyer.html', 'clients/web/admin.html', 'clients/web/prices.html', 'clients/web/supplier.html'];
const pages = new Map();
...
const publicHTML = pages.get('clients/web/index.html');
for (const phrase of [
  'private test canary',
  'no real payments',
  ...
  if (!publicHTML.toLowerCase().includes(phrase)) {
    throw new Error(`clients/web/index.html: missing mandatory canary disclosure: ${phrase}`);
  }
}
if (!/<label\b[^>]*\bfor=["']alpha-role["'][^>]*>/i.test(publicHTML)) {
  throw new Error('clients/web/index.html: alpha role control requires an explicit label');
}
if (!publicHTML.includes(':focus-visible')) {
  throw new Error('clients/web/index.html: visible keyboard focus styling is required');
}
if (!publicHTML.includes('prefers-reduced-motion:reduce')) {
  throw new Error('clients/web/index.html: reduced-motion behavior is required');
}
if (/\.site\s*\{[^}]*display\s*:\s*none/is.test(publicHTML) || !publicHTML.includes('@media (max-width:899px)')) {
  throw new Error('clients/web/index.html: the full site must remain usable at mobile widths');
}

const adminHTML = pages.get('clients/web/admin.html');
if (/\b(?:localStorage|sessionStorage)\b/.test(adminHTML)) {
  throw new Error('clients/web/admin.html: operator bearer credentials must remain memory-only');
}
for (const phrase of [
  ...
  if (!adminHTML.toLowerCase().includes(phrase)) {
    throw new Error(`clients/web/admin.html: missing operator surface: ${phrase}`);
  }
}

const buyerHTML = pages.get('clients/web/buyer.html');
for (const phrase of [
  ...
  if (!buyerHTML.toLowerCase().includes(phrase)) {
    throw new Error(`clients/web/buyer.html: missing buyer contract surface: ${phrase}`);
  }
}
if (/\b(?:localStorage|sessionStorage)\b/.test(buyerHTML)) {
  throw new Error('clients/web/buyer.html: buyer credentials must remain memory-only');
}
...
function cssColor(html, variable) {
  ...
  if (!match) throw new Error(`clients/web/index.html: missing --${variable} color`);
  return match[1];
}
