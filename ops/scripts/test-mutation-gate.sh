...
help="$(bash ops/scripts/mutation-gate.sh --help)"
printf '%s\n' "$help" | rg --fixed-strings 'full       all 125 mutations exactly once' >/dev/null
rg --fixed-strings 'budget="${MERC_MUTATION_WALLCLOCK_SECONDS:-299}"' ops/scripts/mutation-gate.sh >/dev/null
rg --fixed-strings 'default_workers=16' ops/scripts/mutation-gate.sh >/dev/null
if bash ops/scripts/mutation-gate.sh deep >/dev/null 2>&1; then
...
none="$(MERC_MUTATION_BASE=HEAD bash ops/scripts/mutation-gate.sh fast)"
