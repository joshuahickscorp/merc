...
mutants: the shell runner still declares the 125 actual transformations.  This
...
def policy(root: Path) -> dict[str, dict[str, str]]:
    value = load_json(root / "ops" / "scripts" / "mutation-source-policy.json")
...
def validate(manifest: dict[str, Any], inventory: list[tuple[int, str, str]]) -> None:
    ...
    for item in mutations:
        ...
        key = (case_id, source.removeprefix("src/control/"), description)
        if key in actual:
...
def build(root: Path, existing: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for case_id, source, description in inventory:
        ...
        old_is_same = old and old.get("source_target") == f"src/control/{source}" and old.get("description") == description
        ...
        mutations.append({
            "id": case_id,
            "source_target": f"src/control/{source}",
    ...
    for timing_file in timing_files:
        ...
        for campaign in timing_campaigns(timing_file):
            ...
            for record in campaign["records"]:
                ...
                if source != by_id[case_id]["source_target"].removeprefix("src/control/"):
...
def main() -> int:
    ...
    path = (args.manifest if args.manifest else root / "ops" / "scripts" / "mutation-manifest.json").resolve()
