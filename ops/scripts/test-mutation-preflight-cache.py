...
def run(root: Path, *args: str, expect: int = 0) -> subprocess.CompletedProcess[str]:
    result = subprocess.run(
        [sys.executable, str(root / "ops" / "scripts" / "mutation-preflight-cache.py"), *args],
        text=True,
        capture_output=True,
        check=False,
    )
...
def fixture(root: Path) -> tuple[Path, Path, Path]:
    ...
    (root / "src" / "control").mkdir(parents=True)
    for name in (
        "mutation-preflight-cache.py",
        "mutation-test-contracts.py",
        "mutation-contract-observer.py",
    ):
        shutil.copy2(ROOT / "ops/scripts" / name, root / "ops" / "scripts" / name)
    (root / "ops" / "scripts" / "mutation-test-contracts.json").write_text(
        json.dumps(
            {
                "version": 1,
    ...
    (root / "src" / "control" / "price.go").write_text("package control\n", encoding="utf-8")
    (root / "src" / "control" / "store.go").write_text("package control\n", encoding="utf-8")
    (root / "src" / "control" / "price_test.go").write_text(
        "package control\nfunc TestPrice(t *testing.T) {}\nfunc TestPriceDB(t *testing.T) {}\n",
        encoding="utf-8",
    )
    (root / "src" / "control" / "store_test.go").write_text(
...
def main() -> int:
    with tempfile.TemporaryDirectory() as temporary:
        ...
        with (root / "src" / "control" / "store_test.go").open("a", encoding="utf-8") as handle:
