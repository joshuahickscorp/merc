...
scenario_stale_lease_recovery() {
  ...
  # deadWorkerAfter is 180s in src/control/workers.go. We claim tasks by submitting
...
scenario_backup_independent_restore() {
  ...
  [ -x "$ROOT/ops/scripts/backup.sh" ] && [ -x "$ROOT/ops/scripts/restore.sh" ] \
    || die "backup_independent_restore: ops/scripts/backup.sh and ops/scripts/restore.sh must be executable for offsite proof"
  ...
  if ! MERC_BACKUP_RESULT_FILE="$result_file" \
    ...
    bash "$ROOT/ops/scripts/backup.sh" >"$drill_log" 2>"${drill_log}.err"; then
    die "backup_independent_restore: ops/scripts/backup.sh failed: $(head -c 400 "${drill_log}.err" 2>/dev/null || true)"
  fi
  [ -s "$result_file" ] \
  ...
  # writes status=PASS (see ops/scripts/backup.sh verification path).
  ind_download=true
  cipher_ok=true
  ...
  if ! bash "$ROOT/ops/scripts/restore.sh" --db-only --to "$scratch_db" "$backup_id" \
    >"$restore_log" 2>"${restore_log}.err"; then
    die "backup_independent_restore: ops/scripts/restore.sh into scratch db failed: $(head -c 400 "${restore_log}.err" 2>/dev/null || true)"
  fi
...
scenario_stripe_test_matrix() {
  ...
  [ -x "$ROOT/ops/scripts/stripe-sandbox-scenarios.sh" ] \
    || die "stripe_test_matrix: ops/scripts/stripe-sandbox-scenarios.sh is not executable"
  ...
  if ! bash "$ROOT/ops/scripts/stripe-sandbox-scenarios.sh" \
...
scenario_bounded_retry_backoff_audit() {
  ...
  # Policy ceiling is maxTaskRetries=3 in src/control/workers.go.
  max_retry="$(db_scalar "
