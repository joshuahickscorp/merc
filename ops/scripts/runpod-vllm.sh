...
# through ops/scripts/runpod-spend-guard.py, refuses to start while any other pod is
...
# behind.
#
#   bash ops/scripts/runpod-vllm.sh experiment    # GOVERNED: cost cap, enforced lifetime,
#                                             # orphan sweep, verified teardown, receipt
#   bash ops/scripts/runpod-vllm.sh up            # provision, print endpoint, tear down on exit
#   bash ops/scripts/runpod-vllm.sh up --keep     # leave it running (prints the stop command)
#   bash ops/scripts/runpod-vllm.sh list          # what is running right now
#   bash ops/scripts/runpod-vllm.sh reconcile     # detect orphans (exit 1 if any); no terminate
#   bash ops/scripts/runpod-vllm.sh reconcile --terminate-orphans
#                                             # loud terminate of orphans only (explicit)
#   bash ops/scripts/runpod-vllm.sh down <pod-id> # stop one
#   bash ops/scripts/runpod-vllm.sh down-all      # stop everything (the panic button)
...
: "${RUNPOD_API_KEY:?RUNPOD_API_KEY is required (run ops/scripts/merc-credentials.sh)}"
...
PROFILE_PATH="${MERC_VLLM_PROFILE_PATH:-$ROOT/src/control/runtime-profiles/vllm-llama-3.2-1b-instruct-bf16.json}"
...
terminate() {
  ...
  say "  !! run: bash ops/scripts/runpod-vllm.sh down-all"
  return 1
}
...
if d.get('orphans'):
    ...
    print('  refuse quiet success. Inspect with: bash ops/scripts/runpod-vllm.sh reconcile')
  ...
  if [ -n "$orphan_ids" ]; then
    ...
    if [ "${MERC_RUNPOD_TERMINATE_ORPHANS:-0}" = "1" ]; then
      ...
      die "refusing to create a pod while orphans are billing.
       Inspect:  bash ops/scripts/runpod-vllm.sh reconcile
       Terminate orphans only (explicit):  MERC_RUNPOD_TERMINATE_ORPHANS=1 bash ops/scripts/runpod-vllm.sh reconcile --terminate-orphans
       Panic button:  bash ops/scripts/runpod-vllm.sh down-all"
    fi
  fi
...
" 2>/dev/null || true)
  ...
  if [ -n "$preexisting" ]; then
    die "pods are already running and billing: $preexisting
       Stop them first:  bash ops/scripts/runpod-vllm.sh down-all
       Or reconcile:     bash ops/scripts/runpod-vllm.sh reconcile"
  fi
}
...
experiment)
  ...
  # lifetime by ops/scripts/runpod-spend-guard.py, and the conversion lives in Python
...
cleanup() {
  ...
  if [ -n "$POD_ID" ] && { [ "$KEEP" -eq 0 ] || [ "$UP_READY" -eq 0 ]; }; then
    ...
    say "  bash ops/scripts/runpod-vllm.sh down $POD_ID"
  fi
}
...
CREATE=$(python3 "$ROOT/ops/scripts/runpod-create-payload.py" \
...
if [ "$KEEP" -eq 1 ]; then
  say ""
  say "left running. Stop it when done:  bash ops/scripts/runpod-vllm.sh down $POD_ID"
else
  say ""
