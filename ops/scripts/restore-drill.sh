...
docker exec -i "$PG_C" psql -X -v ON_ERROR_STOP=1 -U cx -d cx_drill_source \
  < "$ROOT/src/control/schema.sql" >/dev/null
...
merc_emit_bound_json "$CORRUPT_EVIDENCE_OUT" "ops/scripts/restore-drill.sh" \
  "$ART/corrupt-evidence.json" \
...
# shellcheck source=ops/scripts/lib/write-bound-evidence.sh
. "$ROOT/ops/scripts/lib/write-bound-evidence.sh"
merc_emit_bound_json "$EVIDENCE_OUT" "ops/scripts/restore-drill.sh" \
  "$ART/evidence.json" \
