...
env -i "PATH=$PATH" MERC_RELEASE_DOCTOR_NO_ENV_FILE=1 \
  ...
  bash ops/scripts/release-doctor.sh --check governance >/dev/null \
  || fail "release doctor rejected the canonical approval schema"

cli_receipt="$(cd src/control && go run . release approvals-check --bundle "$valid")"
...
expect_doctor_fail() {
  ...
  if env -i "PATH=$PATH" MERC_RELEASE_DOCTOR_NO_ENV_FILE=1 \
    ...
    bash ops/scripts/release-doctor.sh --check governance >/dev/null 2>&1; then
