...
admission and dispatch call.
    ...
    python3 ops/scripts/branch-state-receipt.py \
...
def authority_state():
    doc = json.loads(read("src/control/runtime-authority.json"))
    profiles = []
...
def dev_authority():
    ...
    out = subprocess.run(
        ...
        cwd=os.path.join(ROOT, "src/control"),
        capture_output=True, text=True,
    )
    if out.returncode != 0:
...
def production_callers(symbol):
    ...
    """
    out = []
    for path in ("control", "src/agent/src"):
        full = os.path.join(ROOT, path)
        if not os.path.isdir(full):
            continue
        for entry in sorted(os.listdir(full)):
...
def main():
    ...
    schema = read("src/control/schema.sql")
    authority = dev_authority()
    api_source = read("src/control/api.go")
    store_jobs_source = read("src/control/store_jobs.go")
    project_submit_tests = read("src/control/project_submit_test.go")
    project_declaration_source = read("src/control/project_declaration.go")
    project_declaration_tests = read("src/control/project_declaration_test.go")
    pricing_source = read("src/control/pricing.go")
    pricing_tests = read("src/control/pricing_test.go")
    pricing_governance_tests = read("src/control/pricing_governance_test.go")
    ...
    receipt = {
        ...
        "capability_wiring": {
            ...
            "coalescing_receipt_provenance": {
                ...
                "handler_money_receipt_test":
                    ...
                    in read("src/control/coalesced_cluster_money_test.go"),
            },
            "coalescing_status": (
            ...
            "project_compiler": {
                ...
                "buyer_approved_probe_required":
                    ...
                    in read("src/control/project_quote.go"),
                ...
                "dependent_step_submit_refused":
                    ...
                    in read("src/control/project_submit.go"),
            ...
            "second_runtime_driver": bool(
                os.path.exists(os.path.join(ROOT, "src/agent/src/runtime_driver.rs"))),
            ...
            # already persisted by the money path (src/control/runtime_cell_cost.go);
            # and a promotion is an entry in runtime_activation_policies carrying
            # the receipt the gate in src/control/runtime_cell_promotion.go derives.
            ...
            "exact_reuse_batch_enabled":
                ...
                read("src/control/exact_reuse_batch.go"),
            "batching_authority": "src/control/batch_policy.go:SelectBatch"
        ...
        "second_runtime_evidence": {
            ...
            "autonomous_agent_chain_tests": [
                name for name in (
                    "src/control/two_agent_enrollment_test.go",
                    "src/control/second_runtime_chain_test.go",
                    "src/control/second_runtime_verification_test.go",
                    "src/control/rejection_economics_test.go",
                    "src/control/receipt_tamper_test.go",
                    "src/control/failure_matrix_test.go",
                    "src/control/failure_matrix_agent_test.go",
    ...
    if args.out:
        ...
        try:
            emit_bound_json(
                path,
                receipt,
                harness="ops/scripts/branch-state-receipt.py",
                repo_root=ROOT,
                build_binary_path=os.path.join(ROOT, "ops/scripts", "branch-state-receipt.py"),
