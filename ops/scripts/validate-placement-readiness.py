...
AUTHORITY = ROOT / "src/control" / "runtime-authority.json"
CONTRACT = ROOT / "ops" / "placement-readiness-contract.json"
EMBED_COMPARATOR = ROOT / "src/control" / "embedding_comparator.go"
RUNTIME_PROFILE = ROOT / "src/control" / "runtime-profiles" / "vllm-llama-3.2-1b-instruct-bf16.json"
SPEND_GUARD = ROOT / "ops/scripts" / "runpod-spend-guard.py"
RUNPOD_VLLM = ROOT / "ops/scripts" / "runpod-vllm.sh"
ORPHAN_TEST = ROOT / "ops/scripts" / "test-runpod-orphan-reconcile.py"
WRITE_BOUND = ROOT / "ops/scripts" / "write-bound-evidence.py"
VALIDATE_BINDING = ROOT / "ops/scripts" / "validate-evidence-binding.py"
SHADOW_SELECTION = ROOT / "src/control" / "runtime_shadow_selection.go"
GOVERNED_COMPARISON = ROOT / "src/control" / "runtime_governed_comparison.go"
CELL_COST = ROOT / "src/control" / "runtime_cell_cost.go"
CELL_COST_TEST = ROOT / "src/control" / "runtime_cell_cost_test.go"
...
def check_matched_model_identity(authority: dict, contract: dict, cells: dict[str, dict]) -> Check:
    ...
    if missing or mismatches:
        return Check(
            ...
            artifact_or_path="src/control/runtime-authority.json + ops/placement-readiness-contract.json",
    ...
    return Check(
        ...
        artifact_or_path="src/control/runtime-authority.json cells; ops/placement-readiness-contract.json",
    )
...
def check_matched_verification(authority: dict, contract: dict, cells: dict[str, dict]) -> Check:
    ...
    if bad:
        return Check(
            ...
            artifact_or_path="src/control/runtime-authority.json; src/control/embedding_comparator.go",
            detail={"problems": bad},
        )
    return Check(
        id=cid,
        ...
        artifact_or_path=(
            ...
            "src/control/embedding_comparator.go:embeddingMeanCosineThreshold/"
...
def check_image_digest(contract: dict) -> Check:
    ...
    if not file_contains(RUNPOD_VLLM, "@sha256:", "must be an immutable OCI digest"):
        return Check(
            ...
            artifact_or_path="ops/scripts/runpod-vllm.sh",
        )
    ...
    if not file_contains(SPEND_GUARD, "@sha256:", "immutable OCI digest"):
        return Check(
            ...
            artifact_or_path="ops/scripts/runpod-spend-guard.py:receipt_rule_refusals",
        )
    withdrawn_note = ""
    ...
    return Check(
        ...
        artifact_or_path=(
            ...
            "ops/scripts/runpod-vllm.sh; ops/scripts/runpod-spend-guard.py"
        ),
    )
...
def check_cuda_model_artifact(authority: dict, contract: dict, cells: dict[str, dict]) -> Check:
    ...
    if not cell:
        return Check(
            ...
            artifact_or_path="src/control/runtime-authority.json",
        )
    ...
    if auth_sha != pinned:
        return Check(
            ...
            artifact_or_path="src/control/runtime-authority.json models[all-minilm-l6-v2].artifacts",
        )
    ...
    return Check(
        ...
        artifact_or_path=(
            "src/control/runtime-authority.json models + vllm-cuda-minilm-embed; "
            "ops/placement-readiness-contract.json; "
...
def check_cost_cap(contract: dict) -> Check:
    ...
    if not SPEND_GUARD.is_file() or not RUNPOD_VLLM.is_file():
        return Check(
            ...
            artifact_or_path="ops/scripts/runpod-spend-guard.py; ops/scripts/runpod-vllm.sh",
        )
    ...
    if "MIN_CAP_USD" not in guard or "budget_seconds" not in guard:
        return Check(
            ...
            artifact_or_path="ops/scripts/runpod-spend-guard.py",
        )
    if "MERC_RUNPOD_CAP_USD" not in vllm or "BUDGET_SECS" not in vllm:
        return Check(
            ...
            artifact_or_path="ops/scripts/runpod-vllm.sh experiment",
        )
    ...
    return Check(
        ...
        artifact_or_path=(
            "ops/scripts/runpod-spend-guard.py:MIN_CAP_USD,budget_seconds; "
            "ops/scripts/runpod-vllm.sh experiment"
        ),
    )
...
def check_startup_deadline() -> Check:
    ...
    if not RUNPOD_VLLM.is_file():
        return Check(
            id=cid,
            state=STATE_UNSATISFIED,
            reason="ops/scripts/runpod-vllm.sh missing",
            artifact_or_path="ops/scripts/runpod-vllm.sh",
        )
    ...
    return Check(
        ...
        artifact_or_path="ops/scripts/runpod-vllm.sh READY loop (seq 1 90; sleep 6)",
    )
...
def check_idle_deadline() -> Check:
    ...
    if not RUNPOD_VLLM.is_file() or not SPEND_GUARD.is_file():
        return Check(
            ...
            artifact_or_path="ops/scripts/runpod-vllm.sh; ops/scripts/runpod-spend-guard.py",
        )
    vllm = RUNPOD_VLLM.read_text(encoding="utf-8")
    ...
    return Check(
        ...
        artifact_or_path=(
            "ops/scripts/runpod-vllm.sh experiment ELAPSED/BUDGET_SECS; "
            "ops/scripts/runpod-spend-guard.py budget_seconds; MERC_RUNPOD_HOLD_SECS"
        ),
    )
...
def check_teardown_and_orphan() -> Check:
    ...
    return Check(
        ...
        artifact_or_path=(
            "ops/scripts/runpod-vllm.sh (trap sweep_all, teardown_verified); "
            "ops/scripts/runpod-spend-guard.py reconcile; "
            "ops/scripts/test-runpod-orphan-reconcile.py; make ci"
        ),
    )
...
def check_spend_receipt_producer_identity() -> Check:
    ...
    if not SPEND_GUARD.is_file() or not WRITE_BOUND.is_file() or not VALIDATE_BINDING.is_file():
        return Check(
            ...
            artifact_or_path="ops/scripts/runpod-spend-guard.py; ops/scripts/write-bound-evidence.py",
        )
    guard = SPEND_GUARD.read_text(encoding="utf-8")
    binding_lib = ROOT / "ops/scripts" / "lib" / "evidence_binding.py"
    if not binding_lib.is_file():
        return Check(
            id=cid,
            state=STATE_UNSATISFIED,
            reason="ops/scripts/lib/evidence_binding.py missing",
            artifact_or_path="ops/scripts/lib/evidence_binding.py",
        )
    ...
    if not wired:
        return Check(
            ...
            artifact_or_path=(
                "ops/scripts/runpod-spend-guard.py receipt command; "
    ...
    if "image_digest" not in guard:
        return Check(
            ...
            artifact_or_path="ops/scripts/runpod-spend-guard.py receipt command",
        )
    return Check(
        id=cid,
        state=STATE_SATISFIED,
        ...
        artifact_or_path=(
            "ops/scripts/runpod-spend-guard.py receipt; "
            "ops/scripts/lib/evidence_binding.py:emit_bound_json; "
...
def check_selector_consumer() -> Check:
    ...
    if missing:
        return Check(
            ...
            artifact_or_path="src/control/runtime_shadow_selection.go",
        )
    ...
    if not file_contains(CELL_COST, "runtime_shadow_selections"):
        return Check(
            ...
            artifact_or_path="src/control/runtime_cell_cost.go",
        )
    if not file_contains(GOVERNED_COMPARISON, "GovernedShadowDecision", "ExcludedCells"):
        return Check(
            ...
            artifact_or_path="src/control/runtime_governed_comparison.go",
        )
    return Check(
        id=cid,
        state=STATE_SATISFIED,
...
def check_cross_hardware_cost_authority() -> Check:
    ...
    if not file_contains(
        ...
        return Check(
            ...
            artifact_or_path="src/control/runtime_cell_cost_test.go",
        )
    ...
    return Check(
        ...
        artifact_or_path=(
            "src/control/runtime_cell_cost.go:comparableHardwareFor; "
            "src/control/runtime_cell_cost_test.go:TestComparableHardwareRefusesToMixMachines"
        ),
    )
...
def check_cuda_cell_not_advertised(cells: dict[str, dict], contract: dict) -> Check:
    ...
    if cell.get("_runtime_lifecycle") in ("ACTIVE", "CANARY"):
        return Check(
            ...
            artifact_or_path="src/control/runtime-authority.json vllm_cuda",
        )
    return Check(
        id=cid,
        state=STATE_SATISFIED,
        ...
        artifact_or_path=(
            "src/control/runtime-authority.json; "
            "src/control/placement_readiness_test.go; "
            "src/control/activation_policy_test.go:cellIsAdvertised"
        ),
    )
    ...
    if bypass_attempts:
        checks.append(
            Check(
                ...
                artifact_or_path="ops/scripts/validate-placement-readiness.py (REFUSED_BYPASS_ENVS)",
