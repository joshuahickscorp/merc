...
One entry point, two surfaces:
    ...
    python3 ops/scripts/alpha-security-suite.py --surface local
    ...
    python3 ops/scripts/alpha-security-suite.py --surface external
    python3 ops/scripts/alpha-security-suite.py --surface external --base-url https://mercmerc.net
...
def _candidate_commit() -> str:
    ...
    try:
        ...
        return candidate_commit(str(Path(__file__).resolve().parents[2]))
    except Exception:
...
def probe_http_go(suite: Suite, tmp: Path) -> None:
    ...
    template = run(
        ["bash", str(ROOT / "ops/scripts" / "ensure-schema-template.sh")],
        env=env,
        timeout=180,
    )
    ...
    if template.returncode != 0 or not template_name.startswith("merc_schema_"):
        suite.record(
            ...
            finding=True,
            severity="P1",
            reproduction="MERC_TEST_DATABASE_URL=… bash ops/scripts/ensure-schema-template.sh",
            location="ops/scripts/ensure-schema-template.sh",
    ...
    if isinstance(attacks, list) and attacks:
        for item in attacks:
            suite.record(
                ...
                location=str(item.get("location") or "src/control/alpha_security_suite_test.go"),
    ...
    suite.record(
        ...
        finding=True,
        severity="P1",
        reproduction="cd src/control && go test -count=1 -run '^TestAlphaSecuritySuite$' .",
        location="src/control/alpha_security_suite_test.go",
        alpha_reachable=False,
...
def probe_authority_corruption(suite: Suite) -> None:
    ...
    suite.record(
        ...
        reproduction="inspect ops/scripts/validate-authorization-matrix.py default-deny tripwire",
        location="ops/scripts/validate-authorization-matrix.py",
        alpha_reachable=False,
    ...
    suite.record(
        ...
        reproduction="python3 ops/scripts/validate-authorization-matrix.py",
        location="ops/scripts/validate-authorization-matrix.py",
        alpha_reachable=False,
    ...
    suite.record(
        ...
        reproduction="json.loads(src/control/runtime-authority.json)['models'] must be non-empty",
        location="src/control/runtime_authority.go //go:embed runtime-authority.json",
        alpha_reachable=False,
    ...
    suite.record(
        ...
        location="src/control/runtime_authority.go",
        alpha_reachable=False,
    ...
    suite.record(
        ...
        location="src/control/crypto.go + src/control/billing.go + src/control/realtime_currency_authority.go",
        alpha_reachable=False,
        detail=proc.stdout[-1500:],
        executed=1,
    )
...
def probe_sandbox_and_ipc(suite: Suite) -> None:
    ...
    enrollment = (ROOT / "src/agent" / "src" / "enrollment.rs").read_text(encoding="utf-8")
    ...
    suite.record(
        ...
        reproduction="inspect src/agent/src/enrollment.rs unix file mode on token write",
        location="src/agent/src/enrollment.rs",
        alpha_reachable=True,
        detail="PermissionsExt + 0600 required for local credential material",
    )

    egress = (ROOT / "src/agent" / "src" / "sandbox_egress.rs").read_text(encoding="utf-8")
    ...
    suite.record(
        ...
        reproduction="inspect src/agent/src/sandbox_egress.rs allowlist CONNECT proxy",
        location="src/agent/src/sandbox_egress.rs",
        alpha_reachable=True,
...
def probe_supply_chain(suite: Suite) -> None:
    ...
    if govuln:
        ...
        suite.record(
            ...
            title="govulncheck on src/control/",
            attempted=True,
            blocked=not finding,
            finding=finding,
            severity="P2" if finding else None,
            reproduction="cd src/control && govulncheck ./...",
            location="src/control/go.mod",
            alpha_reachable=False,
            detail=proc.stdout[-2000:],
        )
    else:
    ...
    if shutil.which("cargo"):
        audit = run(["cargo", "audit", "--version"], cwd=ROOT / "src/agent", timeout=30)
        if audit.returncode == 0:
            proc = run(["cargo", "audit"], cwd=ROOT / "src/agent", timeout=180)
            finding = proc.returncode != 0 and "error" in proc.stdout.lower()
            suite.record(
                attack_id="supply-cargo-audit-agent",
                attack_class="supply_chain",
                title="cargo audit on src/agent/",
                attempted=True,
                blocked=proc.returncode == 0,
                finding=proc.returncode != 0,
                severity="P2" if proc.returncode != 0 else None,
                reproduction="cd src/agent && cargo audit",
                location="src/agent/Cargo.lock",
                alpha_reachable=False,
                detail=proc.stdout[-2000:],
            )
        else:
            # Still parse Cargo.lock for obviously yanked/path deps — record executed.
            lock = (ROOT / "src/agent" / "Cargo.lock").read_text(encoding="utf-8", errors="replace")
            suite.record(
                ...
                finding=False,
                severity=None,
                reproduction="inspect src/agent/Cargo.lock; cargo audit not installed",
                location="src/agent/Cargo.lock",
                alpha_reachable=False,
...
def probe_secrets(suite: Suite) -> None:
    ...
    auditor = ROOT / "ops/scripts" / "secret-exposure-audit.py"
    if auditor.is_file():
        ...
        suite.record(
            ...
            attack_class="supply_chain",
            title="ops/scripts/secret-exposure-audit.py",
            attempted=True,
            blocked=not live_exposed,
            finding=live_exposed,
            severity="P0" if live_exposed else None,
            reproduction="python3 ops/scripts/secret-exposure-audit.py",
            location="ops/scripts/secret-exposure-audit.py",
            alpha_reachable=False,
...
def probe_external_preflight(suite: Suite, client: ExternalClient) -> dict[str, Any]:
    ...
    suite.record(
        ...
        location="src/control/api.go:handleReadyz",
        alpha_reachable=True,
...
def probe_external_tls_and_headers(suite: Suite, client: ExternalClient) -> dict[str, Any]:
    ...
    suite.record(
        ...
        location="ops/deploy/Caddyfile {$SITE_HOST:mercmerc.net}",
        alpha_reachable=True,
    ...
    for name, expected in _CADDY_HEADERS.items():
        ...
        suite.record(
            ...
            location="ops/deploy/Caddyfile header { ... }",
            alpha_reachable=True,
    ...
    suite.record(
        ...
        location="ops/deploy/Caddyfile header Content-Security-Policy",
        alpha_reachable=True,
    ...
    suite.record(
        ...
        location="ops/deploy/Caddyfile header { -Server }",
        alpha_reachable=True,
...
def probe_external_metrics(suite: Suite, client: ExternalClient) -> None:
    ...
    suite.record(
        ...
        location="ops/deploy/Caddyfile @metrics path /metrics respond 404",
        alpha_reachable=True,
        ...
        for entry in routes:
            ...
            if gid == "worker_owned":
                ...
                outcome = record_http_attack(
                    ...
                    path=path,
                    result=result,
                    location="src/control/api.go:authWorker",
                ...
                outcome = record_http_attack(
                    ...
                    path=path,
                    result=result,
                    location="src/control/api.go:authBuyer",
                ...
                for label, hdrs in (
                    ...
                    outcome = record_http_attack(
                        ...
                        path=path,
                        result=result,
                        location="src/control/api.go:authAdmin",
    ...
    *,
    ...
    for attack_id, title, path, sig in cases:
        ...
        outcome = record_http_attack(
            ...
            result=result,
            webhook=True,
            location="src/control/billing.go + src/control/suppliers.go verifyStripeSig",
    ...
    if used_live_authorities:
        ...
        for attack_id, title, path, sig in cross:
            ...
            outcome = record_http_attack(
                ...
                result=result,
                webhook=True,
                location="src/control/billing.go + src/control/suppliers.go distinct endpoint secrets",
        ...
        suite.record(
            ...
            location="src/control/billing.go + src/control/suppliers.go",
            alpha_reachable=True,
            detail=source,
            executed=0,
        )
...
def probe_external_undriven_classes(suite: Suite) -> None:
    ...
    for attack_class, attack_id, title, reason in skipped:
        suite.record(
            ...
            severity=None,
            reproduction=reason,
            location="ops/scripts/alpha-security-suite.py --surface external",
            alpha_reachable=False,
            detail=reason,
            executed=0,
        )
    ...
    if mode == "external":
        ...
        payload = {
            ...
            "honesty": {
                ...
                "reason_readiness_point_not_claimed": (
                    "ops/scripts/validate-readiness.py:external_staging_attack_proven "
        ...
        payload = {
            ...
            "honesty": {
                ...
                "readiness_one_point_claimed": False,
                "reason_readiness_point_not_claimed": (
                    "ops/scripts/validate-readiness.py:external_staging_attack_proven requires "
    ...
    try:
        ...
        identity = {
            ...
            "build_digest": slot_value(
                sha256_file(ROOT / "ops/scripts" / "alpha-security-suite.py")
            ),
            ...
            "harness_revision": slot_value("ops/scripts/alpha-security-suite.py"),
        ...
        write_bound_evidence(
            ...
            identity=identity,
            repo_root=ROOT,
            build_binary_path=ROOT / "ops/scripts" / "alpha-security-suite.py",
        )
...
def run_external(base_url: str) -> int:
    ...
    try:
        ...
        suite.record(
            ...
            finding=True,
            severity="P1",
            reproduction="python3 ops/scripts/alpha-security-suite.py --surface external",
            location="ops/scripts/alpha-security-suite.py",
            alpha_reachable=True,
