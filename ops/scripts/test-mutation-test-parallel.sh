...
plan="$(MERC_MUTATION_WORKERS=16 MERC_MUTATION_WALLCLOCK_SECONDS=1500 \
  bash ops/scripts/mutation-test-parallel.sh --plan)"
printf '%s\n' "$plan"
...
manifest = json.load(open("ops/scripts/mutation-manifest.json", encoding="utf-8"))
...
if MERC_MUTATION_WORKERS=33 bash ops/scripts/mutation-test-parallel.sh --plan >/dev/null 2>&1; then
...
if MERC_MUTATION_PARALLEL_CASE_IDS=126 bash ops/scripts/mutation-test-parallel.sh --plan >/dev/null 2>&1; then
...
MERC_MUTATION_LIST=1 MERC_MUTATION_LIST_DETAIL=1 bash ops/scripts/mutation-test.sh |
  awk -F '\t' '{ print $2 }' | LC_ALL=C sort -u >"$sources_file"
whole_selector="$(python3 ops/scripts/mutation-preflight-cache.py \
  --root . --sources "$sources_file" --selector)"
selector_shards_one="$(python3 ops/scripts/mutation-preflight-cache.py \
  --root . --sources "$sources_file" --selector-shards 2)"
selector_shards_two="$(python3 ops/scripts/mutation-preflight-cache.py \
  --root . --sources "$sources_file" --selector-shards 2)"
...
rg --fixed-strings 'pg_dump --host=127.0.0.1' ops/scripts/mutation-test-parallel.sh >/dev/null
rg --fixed-strings 'pg_restore --host=127.0.0.1' ops/scripts/mutation-test-parallel.sh >/dev/null
...
rg --fixed-strings 'preflight-db-$lane.json' ops/scripts/mutation-test-parallel.sh >/dev/null
rg --fixed-strings 'merc_mutation_preflight_w${lane}' ops/scripts/mutation-test-parallel.sh >/dev/null
rg --fixed-strings 'preflight_db_pids' ops/scripts/mutation-test-parallel.sh >/dev/null
rg --fixed-strings 'terminate_pid_tree' ops/scripts/mutation-test-parallel.sh >/dev/null
rg --fixed-strings 'stop_background_pids' ops/scripts/mutation-test-parallel.sh >/dev/null
rg --fixed-strings 'git -C "$worktree" lfs checkout' ops/scripts/mutation-test-parallel.sh >/dev/null
rg --fixed-strings 'mutation-preflight-cache.py' ops/scripts/mutation-test-parallel.sh >/dev/null
rg --fixed-strings 'MERC_MUTATION_PREFLIGHT_CACHE' ops/scripts/mutation-test-parallel.sh >/dev/null
rg --fixed-strings 'preflight-db.json' ops/scripts/mutation-test-parallel.sh >/dev/null
rg --fixed-strings 'default_workers=16' ops/scripts/mutation-test-parallel.sh >/dev/null
rg --fixed-strings 'weighted-p90-pure-warmup' ops/scripts/mutation-test-parallel.sh >/dev/null
rg --fixed-strings 'source "$ROOT/ops/scripts/lib/mutation-load-preflight.sh"' ops/scripts/mutation-test-parallel.sh >/dev/null
...
rg --fixed-strings 'MERC_TEST_S3_' ops/scripts/mutation-preflight-cache.py >/dev/null
rg --fixed-strings 'newArtifactHarness(' ops/scripts/mutation-preflight-cache.py >/dev/null
rg --fixed-strings 'NewStorageFromEnv(' ops/scripts/mutation-preflight-cache.py >/dev/null
rg --fixed-strings 'NewStorage(' ops/scripts/mutation-preflight-cache.py >/dev/null

python3 - <<'PY'
from pathlib import Path

script = Path("ops/scripts/mutation-test-parallel.sh").read_text(encoding="utf-8")
...
if rg -n -F -- '-timeout=2m' ops/scripts/mutation-test-parallel.sh; then
