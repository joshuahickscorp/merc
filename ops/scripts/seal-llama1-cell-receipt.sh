...
# ops/scripts/seal-embed-cell-receipt.sh.
#
...
# profile_revision is r9 because candle_metal is at r9 in src/control/runtime-authority.json.
...
payload = {
    ...
    "supersedes": {
        ...
        "reasons": [
            ...
            "r4 re-ran merc-agent bench-batch on this host under the current r9 profile and sealed through ops/scripts/write-bound-evidence.py",
        ],
    },
}
...
python3 "$ROOT/ops/scripts/write-bound-evidence.py" \
  --out "$OUT_REL" \
  --harness "src/agent/src/main.rs:run_bench_batch (merc-agent 0.1.0 bench-batch, candle only)" \
...
echo "Next: update src/control/evidence-manifest.json + src/control/runtime-authority.json to re-point candle-metal-llama1-infer authority at $OUT_REL (profile_revision r9, binding_status BOUND, model_artifact_sha256s including the GGUF pin)."
