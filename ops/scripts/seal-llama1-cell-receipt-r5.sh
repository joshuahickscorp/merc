...
OUT_REL="evidence/perf/runtime-benchmarks/candle-metal-llama1-q4-r5.json"
...
"$AGENT_BIN" bench-batch \
  ...
  >"$RAW" 2> >(tee /tmp/merc-llama1-bench-batch-r5.err >&2)
rc=$?
set -e
if [[ $rc -ne 0 ]]; then
  echo "bench-batch failed (exit $rc); see /tmp/merc-llama1-bench-batch-r5.err" >&2
  exit "$rc"
fi

# Preserve raw samples for the deliverable report.
cp "$RAW" /tmp/merc-llama1-bench-batch-r5-raw.json
...
payload = {
    ...
    "harness": "merc-agent bench-batch (candle-metal-llama1-infer r5 settlement-geometry re-measure)",
    "harness_settings": {
        "prompt": prompt,
    ...
    "supersedes": {
        ...
        "reasons": [
            ...
            "r5 re-ran merc-agent bench-batch on this host under profile r9 and sealed through ops/scripts/write-bound-evidence.py under the settlement geometry",
        ],
    },
...
Path("/tmp/merc-llama1-r5-identity.json").write_text(
    json.dumps(
        {
            "engine_build_hash": engine_build_hash,
            "engine_build_identity_policy": engine_build_identity_policy,
...
python3 "$ROOT/ops/scripts/write-bound-evidence.py" \
  --out "$OUT_REL" \
  --harness "src/agent/src/main.rs:run_bench_batch (merc-agent 0.1.0 bench-batch, candle only, r5 settlement geometry)" \
...
echo "identity: /tmp/merc-llama1-r5-identity.json"
echo "raw: /tmp/merc-llama1-bench-batch-r5-raw.json"
