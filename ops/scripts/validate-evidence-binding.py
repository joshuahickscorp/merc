...
(path + ".binding.json").
    ...
    python3 ops/scripts/validate-evidence-binding.py
    python3 ops/scripts/validate-evidence-binding.py --summary-only
...
def collect_citations() -> dict[str, list[str]]:
    ...
    def is_build_artifact(path: Path, rel: str) -> bool:
        ...
        src/control/control and src/agent/target/release/merc-agent embed evidence path
