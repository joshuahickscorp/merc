...
TOOL = ROOT / "ops/scripts" / "mutation-manifest.py"
MANIFEST = ROOT / "ops/scripts" / "mutation-manifest.json"
...
def main() -> int:
    ...
    if len(mutations) != 125 or [item["id"] for item in mutations] != list(range(1, 126)):
        raise SystemExit("manifest does not contain exactly the 125 declared mutations")
    ...
    print("test-mutation-manifest: PASS 125 exact mutations and timing guards")
    return 0
