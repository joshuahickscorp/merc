...
done < <(MERC_MUTATION_LIST=1 bash ops/scripts/mutation-test.sh)
...
if [ ! -f ops/scripts/mutation-manifest.json ]; then
...
result = subprocess.run(
    ["python3", "ops/scripts/mutation-manifest.py", "--root", ".", "--weights"],
...
manifest = json.loads(open("ops/scripts/mutation-manifest.json", encoding="utf-8").read())
...
python3 ops/scripts/verify-lfs-corpus.py --root "$ROOT" >"$run_root/candidate-lfs-verify.log" 2>&1 &
lfs_verify_pid="$!"
...
done < <(cd "${worktrees[0]}" && python3 ops/scripts/mutation-preflight-cache.py \
...
if ! (
  cd "${worktrees[0]}" &&
    python3 ops/scripts/mutation-preflight-cache.py \
...
for ((worker = 1; worker <= workers; worker++)); do
  ...
  (
    ...
    exec bash ops/scripts/mutation-test.sh
  ) >"$log" 2>&1 &
  worker_pids+=("$!")
done
