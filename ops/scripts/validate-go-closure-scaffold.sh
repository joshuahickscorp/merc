...
scripts=(
  ops/scripts/go-closure-deploy.sh
  ops/scripts/go-closure-release-identity.sh
  ops/scripts/go-closure-evidence-proof.sh
  ops/scripts/go-closure-destroy.sh
  ops/scripts/go-closure-rollback-rehearsal.sh
  ops/scripts/go-closure-restart-storm.sh
  ops/scripts/go-closure-canary-rehearsal.sh
  ops/scripts/go-closure-soak.sh
  ops/scripts/validate-go-closure-scaffold.sh
  ops/scripts/lib/go-closure-common.sh
)
...
[ -x "$ROOT/ops/scripts/validate-canary-scenario-receipt.py" ] \
  || die "canary scenario receipt validator is missing or not executable"
[ -x "$ROOT/ops/scripts/validate-agent-restart-receipt.py" ] \
  || die "agent restart receipt validator is missing or not executable"
[ -x "$ROOT/ops/scripts/validate-go-closure-soak-receipt.py" ] \
  || die "GO-closure soak receipt validator is missing or not executable"
[ -x "$ROOT/ops/scripts/validate-backup-verification-receipt.py" ] \
  || die "backup verification receipt validator is missing or not executable"
[ -x "$ROOT/ops/scripts/validate-go-closure-evidence-chain.py" ] \
  || die "final evidence-chain validator is missing or not executable"
python3 -m py_compile "$ROOT/ops/scripts/validate-canary-scenario-receipt.py" \
  || die "canary scenario receipt validator does not compile"
python3 -m py_compile "$ROOT/ops/scripts/validate-agent-restart-receipt.py" \
  || die "agent restart receipt validator does not compile"
python3 -m py_compile "$ROOT/ops/scripts/validate-go-closure-soak-receipt.py" \
  || die "GO-closure soak receipt validator does not compile"
python3 -m py_compile "$ROOT/ops/scripts/validate-backup-verification-receipt.py" \
  || die "backup verification receipt validator does not compile"
python3 -m py_compile "$ROOT/ops/scripts/validate-go-closure-evidence-chain.py" \
...
for contract in \
  ...
  rg -q -- "$contract" "$ROOT/ops/scripts/go-closure-canary-rehearsal.sh" \
    || die "canary rehearsal lacks authority contract $contract"
done
rg -q 'ops/scripts/validate-canary-scenario-receipt.py' "$ROOT/ops/scripts/lib/go-closure-common.sh" \
...
for contract in \
  ...
  rg -q -- "$contract" "$ROOT/ops/scripts/go-closure-restart-storm.sh" \
    || die "restart storm lacks authority contract $contract"
done
rg -q 'ops/scripts/validate-agent-restart-receipt.py' "$ROOT/ops/scripts/lib/go-closure-common.sh" \
...
for contract in \
  ...
  'ops/scripts/validate-go-closure-soak-receipt.py'; do
  rg -q -- "$contract" "$ROOT/ops/scripts/go-closure-soak.sh" \
    || die "soak lacks exact-candidate raw-sample authority contract $contract"
done
rg -q 'ops/scripts/validate-go-closure-soak-receipt.py' "$ROOT/ops/scripts/lib/go-closure-common.sh" \
...
for contract in \
  ...
  'ops/scripts/validate-backup-verification-receipt.py'; do
  rg -q -- "$contract" "$ROOT/ops/scripts/backup.sh" \
...
for contract in 'invocation_result' 'verification_receipt'; do
  rg -q -- "$contract" "$ROOT/ops/scripts/go-closure-rollback-rehearsal.sh" \
    || die "rollback receipt does not embed the validated backup $contract"
done
rg -q 'ops/scripts/validate-backup-verification-receipt.py' "$ROOT/ops/scripts/lib/go-closure-common.sh" \
...
for contract in \
  ...
  rg -q -- "$contract" "$ROOT/ops/scripts/validate-go-closure-evidence-chain.py" \
    || die "final evidence-chain validator lacks authority contract $contract"
done
rg -q 'ops/scripts/validate-go-closure-evidence-chain.py' "$ROOT/ops/scripts/lib/go-closure-common.sh" \
...
done < <(rg -o 'ops/scripts/[a-z0-9-]+[.]sh' "$ROOT/ops/staging/README.md" | sort -u)
pass "all README script references exist"

for script in \
  ops/scripts/go-closure-deploy.sh ops/scripts/go-closure-rollback-rehearsal.sh \
  ops/scripts/go-closure-restart-storm.sh ops/scripts/go-closure-canary-rehearsal.sh \
  ops/scripts/go-closure-soak.sh; do
  rg -q -- '--execute' "$ROOT/$script" || die "$script lacks explicit execution gate"
done
rg -q 'duration.*86400|86400.*duration' "$ROOT/ops/scripts/go-closure-soak.sh" \
...
for count in \
  ...
  scenario="${count%:*}"
  minimum="${count#*:}"
  rg -q "^[[:space:]]+$scenario$" "$ROOT/ops/scripts/go-closure-canary-rehearsal.sh" \
    || die "missing canary scenario $scenario"
  rg -q "required_counts:.*" "$ROOT/ops/scripts/go-closure-canary-rehearsal.sh" \
  ...
  rg -q "$label:$minimum" "$ROOT/ops/scripts/go-closure-canary-rehearsal.sh" \
