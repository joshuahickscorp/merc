...
Runs the commands the receipt already named: `node ops/scripts/site-build.mjs`,
...
BROWSER = ROOT / "ops/scripts" / "website-validation-browser.mjs"
PRODUCER = "ops/scripts/produce-website-validation.py"
...
def main() -> int:
    ...
    built = subprocess.run(
        ["node", str(ROOT / "ops/scripts" / "site-build.mjs")],
        cwd=ROOT,
        check=False,
        capture_output=True,
        text=True,
    ...
    receipt = {
        ...
        "honesty": {
            ...
            "event_date": now[:10],
            "surface_under_test": (
                "loopback static server after `node ops/scripts/site-build.mjs`, "
