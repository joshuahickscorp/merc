...
# RunPod pod provisioned by ops/scripts/runpod-vllm.sh experiment.
#
...
import hashlib, json
p = json.load(open("src/control/runtime-profiles/vllm-llama-3.2-1b-instruct-bf16.json"))
...
if [ ! -x /tmp/gateway_parity_measure.test ] || [ src/control/gateway_parity_measure_test.go -nt /tmp/gateway_parity_measure.test ] || [ src/control/gateway_parity_matrix.go -nt /tmp/gateway_parity_measure.test ]; then
...
set +e
if [ ! -x /tmp/merc_latency_gap.test ] || [ src/control/merc_latency_gap_accounting_test.go -nt /tmp/merc_latency_gap.test ]; then
...
receipt = {
    ...
    "pricing_go_note": (
        "src/control/pricing.go labels nvidia_48gb as ASSUMED ~300 W. This receipt "
        "is the measured replacement candidate; another lane owns pricing.go — "
        "do not edit it here."
    ),
    "would_replace_constant": {
        "file": "src/control/pricing.go",
