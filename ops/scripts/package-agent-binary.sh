...
if [[ "$OS" == "darwin" ]]; then
  ...
  # (src/agent/src/main.rs resolve_sandbox_profile). A stock install without this
...
if [[ "$OS" == "linux" ]]; then
  ...
  VLLM_PROFILE="$ROOT/src/control/runtime-profiles/vllm-llama-3.2-1b-instruct-bf16.json"
...
Install with ops/scripts/install.sh (preferred) or copy merc-agent onto PATH.
