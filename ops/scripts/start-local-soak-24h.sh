...
SESSION="${MERC_SOAK_TMUX_SESSION:-cx-soak-24h}"
...
case "$MODE" in
  run)
    mkdir -p "$RUN"
    exec bash "$ROOT/ops/scripts/local-resilience-rehearsal.sh" soak --duration 86400 --interval 60 \
      > "$LOG" 2>&1
    ;;
  status)
    ...
    ;;
  start) ;;
  *) echo 'usage: ops/scripts/start-local-soak-24h.sh start|status' >&2; exit 2 ;;
esac
...
tmux respawn-pane -k -t "$SESSION":0.0 "exec '$ROOT/ops/scripts/start-local-soak-24h.sh' run"
pane_pid="$(tmux_value '#{pane_pid}')"
