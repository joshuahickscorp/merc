...
cleanup() {
  ...
  if [ -n "$POD_ID" ]; then
    ...
    bash "$ROOT/ops/scripts/runpod-vllm.sh" down "$POD_ID" || {
      echo "!! TEARDOWN FAILED for $POD_ID — run: bash ops/scripts/runpod-vllm.sh down-all" >&2
    }
  fi
  exit $code
}
...
set -o pipefail
if ! bash "$ROOT/ops/scripts/runpod-vllm.sh" up --keep 2>&1 | tee /tmp/runpod-up.log; then
...
"$ROOT/src/agent/target/release/merc-agent" bench-batch \
