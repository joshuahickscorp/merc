...
usage() {
  cat >&2 <<'USAGE'
usage: ops/scripts/go-closure-soak.sh --target local|ssh [--duration SECONDS] [--interval SECONDS] [--iteration] --execute
...
write_bound_jsonl_sidecar(
    sys.argv[1],
    harness="ops/scripts/go-closure-soak.sh",
    repo_root=root,
    build_binary_path=root / "ops/scripts/go-closure-soak.sh",
...
PY
  ...
  if ! python3 "$ROOT/ops/scripts/validate-go-closure-soak-receipt.py" "$GC_EVIDENCE_FILE" \
...
gc_run_on_target "$target" ops/scripts/go-closure-soak.sh "$duration" "$interval" "$qualification"
