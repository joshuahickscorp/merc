...
(signed CI evidence whose own SHA256SUMS then stop matching), src/agent/target/,
...
FROZEN_PATHS = (
    ...
    ("docs/archive/", "historical archive: names are part of the recorded corpus"),
    ...
    ("clients/web/logo/cx-capsule-target.svg", "pinned by path and sha256 in ops/asset-provenance.json"),
    ...
    ("ops/scripts/test-agent-uninstall-legacy.sh",
...
FROZEN_TOKENS = {
    "computexchange-source-fingerprint-v1": "hash domain separator (src/control/evidence.go); historical v1 scheme",
    "computexchange-source-fingerprint-v2": "hash domain separator (src/control/evidence.go); LFS OID+size binding",
    ...
    "cx-enrollment-request-v1":
        "hashed into every enrollment request id (src/control/enrollment.go: "
...
BLOCKED_SUBSTRINGS = {
    ...
    "ops/scripts/cx": "the cx CLI binary name",
