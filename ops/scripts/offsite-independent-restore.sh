...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'USAGE'
usage: ops/scripts/offsite-independent-restore.sh [--check|--execute] [--source isolated|droplet]
...
USAGE
      ...
      ;;
    *)
      echo "usage: ops/scripts/offsite-independent-restore.sh [--check|--execute] [--source isolated|droplet]" >&2
      exit 2
      ;;
  esac
  shift
...
[ "$SOURCE" = isolated ] || [ "$SOURCE" = droplet ] || {
  echo "usage: ops/scripts/offsite-independent-restore.sh [--source isolated|droplet]" >&2
  exit 2
}
...
droplet_remote() {
  ...
  python3 "$ROOT/ops/scripts/lib/droplet-remote.py" --timeout "$timeout" "$@"
}
...
resolve_schema() {
  if [ -f "$ROOT/src/control/schema.sql" ]; then
    SCHEMA_FILE="$ROOT/src/control/schema.sql"
    return
  fi
  SCHEMA_FILE="$WORK/schema.sql"
  git -C "$ROOT" show HEAD:src/control/schema.sql >"$SCHEMA_FILE" \
    || die "src/control/schema.sql is not materialized and git show failed"
  [ -s "$SCHEMA_FILE" ] || die "git show HEAD:src/control/schema.sql produced an empty file"
}
...
droplet_presign_put() {
  ...
  url="$(python3 "$ROOT/ops/scripts/lib/r2-presign-put.py" \
...
produce_and_upload_droplet() {
  ...
  droplet_remote 60 push "$ROOT/ops/scripts/lib/droplet-offsite-produce.sh" "$remote_script" \
...
python3 "$ROOT/ops/scripts/validate-backup-verification-receipt.py" \
...
merc_emit_bound_json \
  ...
  "ops/scripts/offsite-independent-restore.sh" \
  "$VERIFY_PAYLOAD" \
