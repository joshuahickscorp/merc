...
Candidate canary authority belongs to ops/scripts/go-closure-canary-rehearsal.sh,
...
def _endpoint_host_port(endpoint):
    ...
    ``http://127.0.0.1:9000`` (what src/control/storage.go and compose files use).
    """
    if "://" in endpoint:
...
def cap_local_runtime():
    ...
    agent = os.path.join(REPO, "src", "agent", "target", "release", "merc-agent")
...
LANES = [
    ...
    {"id": "embeddings", "needs": ["database", "object_store", "local_runtime"],
     "cmd": go_test("TestBillingCustomerCanonicalSchema|TestExactReuse"), "cwd": "src/control",
     "retained_evidence": {
         "path": "evidence/canary/real-runtime-embed.json",
         "lane": "batch_embeddings",
         "result_field": "result",
    ...
    {"id": "image_generation", "needs": [],
     "cmd": go_test("TestImage"), "cwd": "src/control",
     "note": "governance only; no image runtime exists"},
    {"id": "lora", "needs": [],
     "cmd": go_test("TestLoRA"), "cwd": "src/control",
    ...
    {"id": "external_model_onboarding", "needs": ["real_inference_runtime"],
     "cmd": ["bash", "ops/scripts/onboard-model-canary.sh"], "cwd": ".",
    ...
    {"id": "refunds_disputes", "needs": ["database"],
     "cmd": go_test("TestResolveDispute|TestReversal"), "cwd": "src/control",
    ...
    {"id": "failure_recovery", "needs": ["database"],
     "cmd": go_test("TestStuckRunningJobs|TestRescueStuckJob"), "cwd": "src/control",
    ...
    {"id": "backup_restore", "needs": [],
     "cmd": ["bash", "ops/scripts/test-backup-schedule.sh"], "cwd": ".",
     "note": "backup scheduling and envelope"},
    {"id": "buyer_dashboard", "needs": ["database"],
     "cmd": ["node", "ops/scripts/buyer-dashboard-live.mjs"], "cwd": ".",
    ...
    {"id": "supplier_console", "needs": [],
     "cmd": ["node", "ops/scripts/test-supplier-console.mjs"], "cwd": ".",
    ...
    {"id": "python_sdk", "needs": ["database", "object_store", "local_runtime"],
     "cmd": ["python3", "ops/scripts/sdk-live-python.py"], "cwd": ".",
    ...
    {"id": "typescript_sdk", "needs": ["database", "object_store", "local_runtime"],
     "cmd": ["node", "ops/scripts/sdk-live-typescript.mjs"], "cwd": ".",
    ...
    {"id": "alerts", "needs": [],
     "cmd": ["node", "ops/scripts/site-build.mjs"], "cwd": ".",
...
def main() -> int:
    ...
    report = {
        ...
        "candidate_canary_authority": "ops/scripts/go-closure-canary-rehearsal.sh",
    ...
    _scripts = os.path.join(REPO, "ops", "scripts")
    ...
    try:
        emit_bound_json(
            out,
            report,
            harness="ops/scripts/private-canary.py",
            repo_root=REPO,
            build_binary_path=os.path.join(REPO, "ops", "scripts", "private-canary.py"),
