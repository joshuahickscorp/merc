...
Run ONLY as the workload of ops/scripts/runpod-vllm.sh experiment, which owns the
...
# The verification contract, from src/control/embedding_comparator.go.
...
def main() -> int:
    ...
    if not base or not key:
        print(
            ...
            "must run as the workload of ops/scripts/runpod-vllm.sh experiment, which owns "
    ...
    art = {
        ...
        "cost_comparison": {
            ...
            "authority": "src/control/runtime_cell_cost.go:comparableHardwareFor",
            "test": "src/control/runtime_cell_cost_test.go:TestComparableHardwareRefusesToMixMachines",
            "why": (
    ...
    try:
        identity = default_bound_identity(
            ROOT,
            harness_revision=f"ops/scripts/cuda-embed-arm-measure.py@{harness_sha[:16]}",
