...
a license clearance that does not exist.
    ...
    python3 ops/scripts/generate-sbom.py --out evidence/state/sbom.json
"""
import argparse, json, subprocess, sys, hashlib, os

def go_modules(root):
    out = subprocess.run(["go", "list", "-m", "-json", "all"], cwd=os.path.join(root, "src", "control"),
...
def rust_packages(root):
    out = subprocess.run(["cargo", "metadata", "--format-version", "1", "--offline"],
                         cwd=os.path.join(root, "src", "agent"), capture_output=True, text=True)
    if out.returncode != 0:
        return []
    comps = []
    ...
    doc = {
        ...
        "metadata": {
            ...
            "properties": [
                ...
                {"name": "merc:generator", "value": "ops/scripts/generate-sbom.py"},
            ],
        },
        "components": comps,
        "merc_honesty": {
