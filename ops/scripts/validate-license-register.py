...
PRICING = ROOT / "src/control" / "pricing.go"
...
def repricing_model_ids() -> list[str]:
    text = tracked_text("src/control/pricing.go")
    ...
    if not block:
        fail("could not locate repricingBenchmarks in src/control/pricing.go")
...
def validate_generated_inventory() -> None:
    ...
    if not GENERATED.is_file():
        fail("missing docs/generated/license-inventory.json; run ops/scripts/generate-license-inventory.py")
    try:
