...
#
#   ops/scripts/stripe-sandbox-connect.sh
#   ops/scripts/stripe-sandbox.sh connect
#
...
if [ "${1:-}" = "-h" ] || [ "${1:-}" = "--help" ]; then
  cat >&2 <<'EOF'
usage: ops/scripts/stripe-sandbox-connect.sh [--self-test]
...
Also accepted as: ops/scripts/stripe-sandbox.sh connect
EOF
  exit 2
fi
...
if [ "${1:-}" = "--self-test" ]; then
  python3 "$ROOT/ops/scripts/lib/stripe-sandbox-connect.py" --self-test
  exit $?
fi
...
# Full-matrix orchestration (ops/scripts/stripe-sandbox.sh matrix) sets
...
python3 "$ROOT/ops/scripts/lib/stripe-sandbox-connect.py"
