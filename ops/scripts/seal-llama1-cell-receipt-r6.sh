...
# bench-batch) and writes the authority through ops/scripts/write-bound-evidence.py.
#
# Opt-in only:
#   MERC_LLAMA1_CELL_PERF=1 ./scripts/seal-llama1-cell-receipt-r6.sh
#
# Prerequisites:
#   - src/agent/target/release/merc-agent (or AGENT_BIN) built with metal
...
OUT_REL="evidence/perf/runtime-benchmarks/candle-metal-llama1-q4-r6.json"
...
# Authority pin for the generation GGUF (src/control/runtime-authority.json + agent models.rs).
...
"$AGENT_BIN" bench-batch \
  ...
  >"$RAW" 2> >(tee /tmp/merc-l12/llama1-bench-batch-r6.err >&2)
rc=$?
set -e
fi
if [[ $rc -ne 0 ]]; then
  echo "bench-batch failed (exit $rc); see /tmp/merc-l12/llama1-bench-batch-r6.err" >&2
  exit "$rc"
fi

# Preserve raw samples for the deliverable report.
cp "$RAW" /tmp/merc-l12/llama1-bench-batch-r6-raw.json
...
payload = {
    ...
    "harness": "merc-agent bench-batch (candle-metal-llama1-infer r6 settlement-geometry re-measure)",
    "harness_settings": {
        "prompt": prompt,
    ...
    "supersedes": {
        ...
        "reasons": [
            ...
            "r6 re-ran merc-agent bench-batch on this host under profile r9 with the current agent binary and sealed through ops/scripts/write-bound-evidence.py",
        ],
    },
...
Path("/tmp/merc-l12/llama1-r6-identity.json").write_text(
    json.dumps(
        {
            "engine_build_hash": engine_build_hash,
            "engine_build_identity_policy": engine_build_identity_policy,
...
python3 "$ROOT/ops/scripts/write-bound-evidence.py" \
  --out "$OUT_REL" \
  --harness "src/agent/src/main.rs:run_bench_batch (merc-agent 0.1.0 bench-batch, candle only, r6 settlement geometry)" \
...
echo "identity: /tmp/merc-l12/llama1-r6-identity.json"
echo "raw: /tmp/merc-l12/llama1-bench-batch-r6-raw.json"
echo "Next: measure whole-package power, update evidence-manifest + runtime-authority + pricing.go."
