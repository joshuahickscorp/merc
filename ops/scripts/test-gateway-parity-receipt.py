...
src/control/gateway_parity_harness.go (schema_version=2, gate_version=
gateway-parity-gate-v4). The withdrawn v1 shape (ops/scripts/gateway-parity.py)
...
window (aligned with src/control/runtime_cell_performance.go
...
cannot greenwash a gate that only the v2 harness may pass.
    ...
    python3 ops/scripts/test-gateway-parity-receipt.py
    python3 ops/scripts/test-gateway-parity-receipt.py --self-test-only
...
# Keep aligned with src/control/runtime_cell_performance.go:benchmarkRevalidationWindow.
