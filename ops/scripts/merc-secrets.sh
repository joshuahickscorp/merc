...
#
# Separate from ops/scripts/merc-credentials.sh on purpose. That script serves the
...
#
#   bash ops/scripts/merc-secrets.sh            # prompt for anything missing
#   bash ops/scripts/merc-secrets.sh --check    # verify what is already stored
#
...
check_alert_receiver() {
  ...
  # ops/scripts/test-alert-delivery.sh proves real delivery when you want it.
...
[ "$alert_ok" -eq 1 ] \
  && ok "alerts: receiver recorded; run ops/scripts/test-alert-delivery.sh to prove delivery" \
  || skip "alerts: no receiver, a page would land nowhere"

say ""
say "next:"
[ "$cf_ok" -eq 1 ] && say "  bash ops/scripts/cloudflare-zones.sh list"
[ "$stripe_ok" -eq 1 ] && say "  bash ops/scripts/merc-secrets.sh --check   # re-verify any time"
