...
usage() {
  echo "usage: ops/scripts/go-closure-canary-rehearsal.sh --target local|ssh --check|--execute" >&2
  exit 2
}

validate_scenario_receipt() {
  local file="$1" scenario="$2" minimum="$3" scenario_started_at="$4" checked_at="$5"
  python3 "$ROOT/ops/scripts/validate-canary-scenario-receipt.py" "$file" \
...
host_rehearsal() {
  ...
  for index in "${!scenarios[@]}"; do
    ...
    merc_emit_bound_json "$receipt" "ops/scripts/canary-scenario-driver.sh" "$payload" \
...
gc_run_on_target "$target" ops/scripts/go-closure-canary-rehearsal.sh "$operation"
