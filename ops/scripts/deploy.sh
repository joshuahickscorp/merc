...
# shellcheck source=ops/scripts/lib/compose-prod.sh
. "$ROOT/ops/scripts/lib/compose-prod.sh"
PULL=0
[ "${1:-}" != --pull ] || PULL=1
[ $# -le 1 ] || { echo "usage: ops/scripts/deploy.sh [--pull]" >&2; exit 2; }
...
# Keep backup health dir consistent with ops/scripts/backup.sh / systemd timer.
