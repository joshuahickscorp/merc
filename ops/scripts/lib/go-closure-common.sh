...
gc_atomic_json() {
  ...
  . "$GC_ROOT/ops/scripts/lib/write-bound-evidence.sh"
  merc_emit_bound_json "$destination" "ops/scripts/lib/go-closure-common.sh:gc_atomic_json" \
    "$temporary" \
...
gc_run_on_target() {
  ...
  gc_validate_absolute_path STAGING_DEPLOYMENT_ROOT
  [[ "$script" =~ ^ops/scripts/go-closure-[a-z0-9-]+\.sh$ ]] || gc_die "unsafe target script"
  local arg
...
gc_sync_bundle() {
  ...
  local files=(
    ...
    ops/scripts/validate-backup-verification-receipt.py
    ...
    ops/scripts/validate-agent-restart-receipt.py
    ops/scripts/go-closure-canary-rehearsal.sh
    ops/scripts/validate-canary-scenario-receipt.py
    ops/scripts/go-closure-soak.sh
    ops/scripts/validate-go-closure-soak-receipt.py
    ops/scripts/validate-go-closure-evidence-chain.py
