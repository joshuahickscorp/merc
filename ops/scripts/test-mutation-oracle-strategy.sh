...
python3 ops/scripts/test-mutation-suite-observer.py

# ---------------------------------------------------------------------------
# Structural: rename, aliases, safe bare default, deep tier uses oracle.
# ---------------------------------------------------------------------------
default_line="$(rg -n 'MERC_MUTATION_TEST_STRATEGY="\$\{MERC_MUTATION_TEST_STRATEGY:-' ops/scripts/mutation-test.sh | head -n1)"
printf '%s\n' "$default_line" | rg --fixed-strings ':-adaptive}' >/dev/null || {
  echo "bare default is not adaptive: $default_line" >&2
  exit 1
}
rg --fixed-strings 'MERC_MUTATION_TEST_STRATEGY=oracle' ops/scripts/mutation-test.sh >/dev/null
rg --fixed-strings 'full|whole-suite)' ops/scripts/mutation-test.sh >/dev/null
rg --fixed-strings 'strategy=oracle' ops/scripts/mutation-gate.sh >/dev/null
# The gate tier name "full" must remain a tier, not be confused with the strategy.
rg --fixed-strings 'full       all 125 mutations exactly once' ops/scripts/mutation-gate.sh >/dev/null
# Deep is the only tier that selects the oracle strategy.
python3 - <<'PY'
from pathlib import Path
text = Path("ops/scripts/mutation-gate.sh").read_text(encoding="utf-8")
...
alias_help="$(
  MERC_MUTATION_LIST=1 MERC_MUTATION_TEST_STRATEGY=full bash ops/scripts/mutation-test.sh 2>&1 | head -n1 || true
)"
# Listing must succeed under the historical alias.
list_count="$(MERC_MUTATION_LIST=1 MERC_MUTATION_TEST_STRATEGY=full bash ops/scripts/mutation-test.sh | wc -l | tr -d ' ')"
[ "$list_count" = "125" ] || {
  echo "full alias did not list 125 mutations (got $list_count)" >&2
  exit 1
}
list_count_oracle="$(MERC_MUTATION_LIST=1 MERC_MUTATION_TEST_STRATEGY=oracle bash ops/scripts/mutation-test.sh | wc -l | tr -d ' ')"
[ "$list_count_oracle" = "125" ] || {
  echo "oracle strategy did not list 125 mutations (got $list_count_oracle)" >&2
  exit 1
}
list_count_ws="$(MERC_MUTATION_LIST=1 MERC_MUTATION_TEST_STRATEGY=whole-suite bash ops/scripts/mutation-test.sh | wc -l | tr -d ' ')"
[ "$list_count_ws" = "125" ] || {
  echo "whole-suite alias did not list 125 mutations (got $list_count_ws)" >&2
  exit 1
}

# ---------------------------------------------------------------------------
# Sandbox harness: tiny control package + one mutation case path.
# We do not run the real 125-case campaign. We exercise the oracle preflight and
...
mkdir -p "$PROOF/ops/scripts" "$PROOF/src/control"
# Scripts the oracle path needs.
cp ops/scripts/mutation-test.sh \
  ops/scripts/mutation-suite-observer.py \
  "$PROOF/ops/scripts/"
...
cp ops/scripts/mutation-contract-observer.py \
  ops/scripts/mutation-test-contracts.py \
  ops/scripts/mutation-test-contracts.json \
  ops/scripts/mutation-preflight-cache.py \
  ops/scripts/with-isolated-test-db.sh \
  "$PROOF/ops/scripts/" 2>/dev/null || true
...
python3 - "$PROOF/ops/scripts/mutation-test.sh" <<'PY'
from pathlib import Path
import sys
...
cat >"$PROOF/src/control/go.mod" <<'EOF'
module merc/src/control

go 1.22
EOF

cat >"$PROOF/src/control/money_nanos.go" <<'EOF'
package control
...
cat >"$PROOF/src/control/money_nanos_test.go" <<'EOF'
package control
...
# ---------------------------------------------------------------------------
cat >"$PROOF/src/control/unrelated_red_test.go" <<'EOF'
package control
...
red_out="$(
  cd "$PROOF" &&
    ...
    bash ops/scripts/mutation-test.sh 2>&1
)"
red_status=$?
set -e
...
rm -f "$PROOF/src/control/unrelated_red_test.go"
...
if ! grep -q 'return a + b' "$PROOF/src/control/money_nanos.go"; then
