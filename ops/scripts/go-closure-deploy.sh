...
usage() {
  cat >&2 <<'USAGE'
usage: ops/scripts/go-closure-deploy.sh --target local|ssh --activate candidate|prior --check|--execute
...
gc_run_on_target "$target" ops/scripts/go-closure-deploy.sh "$operation" "$activation"
