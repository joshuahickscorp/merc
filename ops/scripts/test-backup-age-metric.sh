...
# --- Fresh backup signal (simulates ops/scripts/backup.sh post-verify write) ---
FRESH_TS="$(date -u +%s)"
...
DRY_RUN_OK=false
if grep -q -- '--dry-run' "$ROOT/ops/scripts/backup.sh"; then
  set +e
  ...
  MERC_BACKUP_ENCRYPTION_RECIPIENT="age1dryrunqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq" \
    bash "$ROOT/ops/scripts/backup.sh" --dry-run >"$ART/backup-dry-run.log" 2>&1
  DRY_RC=$?
  set -e
...
merc_emit_bound_json "$EVIDENCE_OUT" "ops/scripts/test-backup-age-metric.sh" \
  "$ART/evidence.json" \
