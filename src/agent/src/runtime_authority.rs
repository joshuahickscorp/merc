...
/// The capability manifest digest, mirroring src/control/capability_manifest.go.
///
...
// Keep a repository layout move from changing the identity of an otherwise
// identical runtime profile. Arbitrary provenance values remain digest-bound.
fn capability_source_identity(value: &str) -> &str {
    match value {
        "control/runtime-authority.json" | "src/control/runtime-authority.json" => {
...
fn profile_capability_lines(authority: &Authority, profile: &RuntimeProfile) -> Vec<String> {
    let mut lines = vec![cap_line(&[
        ...
        capability_source_identity(&profile.source_identity),
        &profile.device,
...
mod tests {
    ...
    /// failure rather than a production refusal. src/control/capability_manifest_test.go
    ...
    fn capability_matrix_digest_matches_the_control_plane() {
        assert_eq!(
            ...
            "the agent's capability matrix digest no longer matches the pinned \
             ...
             the pin in BOTH src/control/capability_manifest_test.go and here"
        );
    }
    ...
    #[test]
    fn authority_layout_alias_is_canonicalized_for_capability_identity() {
        assert_eq!(
            super::capability_source_identity("src/control/runtime-authority.json"),
            "control/runtime-authority.json"
        );
        assert_eq!(
            super::capability_source_identity("control/runtime-authority.json"),
            "control/runtime-authority.json"
        );
        assert_eq!(
            super::capability_source_identity("other-authority.json"),
            "other-authority.json"
        );
    }
}
