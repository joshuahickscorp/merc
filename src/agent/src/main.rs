...
fn reexec_under_sandbox_if_needed() {
    ...
    let (_proxy_child, proxy_url) = match sandbox_egress::spawn_proxy_process(&exe, &allowlist) {
        ...
        Err(err) => {
            sandbox_wrap_failed(&format!(
                "could not start allowlisted egress proxy ({err}); without it seatbelt cannot reach remote src/control/artifact hosts"
            ));
            return;
        }
    };
...
enum Command {
    ...
    /// ops/scripts/engine-parity-metal-embed-measure.py to seal a BOUND receipt.
    BenchEmbedParity {
        #[arg(long, default_value = "all-minilm-l6-v2")]
        model: String,
        /// llama-server started with `--embedding --pooling mean` on the pinned GGUF.
    ...
    /// ops/scripts/seed-batch-infer-honeypot.sh / control seed.
    HoneypotAnswer {
        #[arg(long, default_value = "llama-3.2-1b-instruct-q4")]
        model: String,
...
async fn run_honeypot_answer(model: &str, max_tokens: u32, prompt: &str) -> Result<()> {
    ...
    let out = serde_json::json!({
        ...
        // whatever order that serializer chooses, and src/control/verification.go
...
/// declaration order. `src/control/verification.go` compares batch_infer honeypots
...
async fn run_bench_embed_parity(
    ...
    let payload = serde_json::json!({
        ...
        "corpus": {
            ...
            "source": "EMBED_BENCH_CORPUS in src/agent/src/main.rs",
        },
        "quality": {
            "verification": "cosine",
            "gate": COSINE_GATE,
...
/// Locate and invoke `ops/scripts/real-engine.sh` from the repo root or CWD.
fn run_real_engine_script(action: &str) -> Result<()> {
    let candidates = [
        PathBuf::from("ops/scripts/real-engine.sh"),
        PathBuf::from("../../ops/scripts/real-engine.sh"),
        // When the binary is invoked via absolute path from elsewhere.
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../ops/scripts/real-engine.sh"),
    ];
    let script = candidates
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "ops/scripts/real-engine.sh not found (cwd={:?}); start llama-server manually",
...
mod tests {
    ...
    fn honeypot_known_answer_is_in_worker_wire_order_not_alphabetical() {
        ...
        // src/control/verification.go compares batch_infer honeypots with bytes.Equal
    ...
    fn sha256_hex_matches_known_test_vector() {
        ...
        assert_eq!(got.len(), 64, "must be 64 lowercase hex chars  -  the exact shape src/control/store.go's nullSHA256Hex requires");
    }
