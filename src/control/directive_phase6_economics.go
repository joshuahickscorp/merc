...
func BuildMetalLocalPhase6Economics() Phase6EconomicsReceipt {
	...
	// Electricity rate is policy-defaulted (same constant as src/control/pricing.go),
	...
	energyUSDQty := knownQty(
		...
		boundEnergyAuthorityPath+" + src/control/pricing.go:defaultElectricityUSDPerKWh",
		fmt.Sprintf(
...
func metalLocalCategoryInventory() []DirectiveCostCategory {
	return []DirectiveCostCategory{
		...
		},
		{
			...
			Source:    "src/control/provider_cost_authority.go (owned/community supply)",
		...
		},
		{
			...
			Notes: "CostSchedule publishes defaulted AWS rates with provenance (src/control/cost_schedule.go) " +
		...
		},
		{
			...
			Notes: fmt.Sprintf(
				"BOUND GPU-domain energy: %.9f J/verified_outcome (IOReport); "+
					"dollar conversion uses defaulted $%.2f/kWh (src/control/pricing.go) and is partial only",
				boundJoulesPerVerifiedOutcome, defaultElectricityUSDPerKWh,
			),
