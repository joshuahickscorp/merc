...
// ops/scripts/alpha-security-suite.py can bind them into evidence. The test
	...
	for _, class := range matrix.RouteClasses {
		...
		for _, pattern := range class.Routes {
			...
			s.record(alphaAttack{
				...
				Location:       "src/control/api.go:Server.Routes + authBuyer/authWorker/authAdmin/webhook",
			...
			case "operator":
				...
				s.record(alphaAttack{
					...
					Location:       "src/control/api.go:authAdmin + admin_credentials",
					AlphaReachable: true, Detail: got.body,
				})
			}
			checked++
...
func (s *alphaSuite) attackIdentityAndPrivilege() {
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/api.go:handleGetJob + store.GetJob buyer_id predicate",
		AlphaReachable: true, Detail: got.body,
	})
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/api.go:handleFileDispute + RecordDispute",
		AlphaReachable: true, Detail: got.body,
	})
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/api.go:handleWorkerCommit + store.completeTaskTx claimed_by fence",
		AlphaReachable: true, Detail: got.body,
	})
	...
	if successCode(got.code) {
		...
		if err != nil || stored != s.workerAID {
			s.record(alphaAttack{
				...
				Location:       "src/control/api.go:handleWorkerRegister cap.WorkerID = auth.WorkerID",
				AlphaReachable: true, Detail: got.body,
			})
		} else {
			s.record(alphaAttack{
				...
				Reproduction:   "POST /v1/worker/register with worker A token and worker B id in body",
				Location:       "src/control/api.go:handleWorkerRegister",
		...
		if raw != "" {
			...
			if err == nil && auth.IsAdmin {
				s.record(alphaAttack{
					...
					Location:       "src/control/api.go:handleCreateKey + store.CreateAPIKey",
				...
				s.record(alphaAttack{
					...
					Location:       "src/control/api.go:handleCreateKey",
	...
	if s.sessionA != "" {
		...
		s.record(alphaAttack{
			...
			Location:       "src/control/accounts.go:RevokeSession + LookupSession revoked=false",
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/admin_authority.go:adminSourceAllowed",
		AlphaReachable: true, Detail: got.body,
	})
	...
	if err == nil {
		...
		s.record(alphaAttack{
			...
			Location:       "src/control/api.go:authWorker + LookupWorkerToken",
			AlphaReachable: true, Detail: got.body,
		})
	}
}
...
func (s *alphaSuite) attackWebhooks() {
	...
	for _, tc := range cases {
		...
		s.record(alphaAttack{
			...
			Location:       "src/control/billing.go:handleStripeWebhook + src/control/suppliers.go:handleConnectWebhook + verifyStripeSig",
			AlphaReachable: true, Detail: got.body,
		})
	}
	...
	if strings.Contains(got.body, "invalid stripe signature") {
		s.record(alphaAttack{
			...
			Location:       "src/control/billing.go:handleStripeWebhook",
...
func (s *alphaSuite) attackMoney() {
	...
	for _, tc := range money {
		...
		s.record(alphaAttack{
			...
			Location:       "src/control/api.go Routes + authAdmin/authBuyer",
			AlphaReachable: true, Detail: got.body,
		})
	}
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/prepaid.go:handleBillingTopup decodeStrictJSONObject",
		AlphaReachable: true, Detail: got.body,
	})
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/store_tasks.go:completeTaskTx claimed_by fence",
		AlphaReachable: true, Detail: got.body,
	})
}
...
func (s *alphaSuite) attackFXAndAmounts() {
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/api.go:createJob max_usd finite non-negative",
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/api.go:createJob math.IsInf/IsNaN",
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/api.go:jobSubmit + decodeStrictJSONObject",
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/realtime_currency_authority.go + quote path",
	...
	for _, amt := range []string{"-10.00", "0", "999999999999.99"} {
		...
		s.record(alphaAttack{
			...
			Location:       "src/control/prepaid.go:handleBillingTopup ParseMajorToMinorExact",
			AlphaReachable: true, Detail: got.body,
		})
	}
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/prepaid.go:handleBillingTopup requestedCurrency.Equal(settlement)",
		AlphaReachable: true, Detail: got.body,
	})
}
...
func (s *alphaSuite) attackContainmentAndInput() {
	...
	for _, p := range []string{
		...
		s.record(alphaAttack{
			...
			Reproduction:   "GET " + p,
			Location:       "src/control/api.go:rejectPathTraversal + handleSiteAsset",
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/project_compile_api.go:cleanProjectArchivePath",
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/store_tasks.go:completeTaskTx expectedResultKey",
		AlphaReachable: true, Detail: got.body,
	})
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/project_declaration.go:validProjectArtifactRef",
		AlphaReachable: true, Detail: got.body,
	})
}
...
func (s *alphaSuite) attackConcurrency() {
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/scheduler.go:ClaimTasksTx FOR UPDATE SKIP LOCKED",
		AlphaReachable: true,
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/store_payouts.go:ReleasePayoutTx + ClaimPayout FOR UPDATE",
...
func (s *alphaSuite) attackAuthorityFailClosed() {
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/store_payouts.go:ReleasePayoutTx + reservePayoutFunding",
		AlphaReachable: false, Detail: got.body,
	})
...
func (s *alphaSuite) attackResourceExhaustion() {
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/api.go:capBody + requestBodyLimit",
		AlphaReachable: true, Detail: rec.Body.String(),
	})
	...
	s.record(alphaAttack{
		...
		Location:       "src/control/billing.go:io.LimitReader 1<<20",
