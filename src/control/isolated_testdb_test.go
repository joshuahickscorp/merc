...
// understands (ops/scripts/with-isolated-test-db.sh). When set to a schema-stamped
...
func validateIsolatedTestDBTemplate(name string) error {
	...
	if strings.HasPrefix(name, schemaTemplateNamePrefix) {
		...
		if name != want {
			return fmt.Errorf("%s=%q is stale for the embedded src/control/schema.sql (want %s); refusing to clone an old schema",
