...
// src/control/. CPU and alloc profiles land next to that file (not LFS).
...
const (
	...
	// Production agent cadence (src/agent/src/vllm.rs HEARTBEAT_INTERVAL).
...
func TestControlPlaneHotPathProfile(t *testing.T) {
	...
	command := "cd src/control && MERC_CONTROL_PLANE_PROFILE=1 MERC_TEST_DATABASE_URL=postgres://cx:cx@localhost:5432/cx?sslmode=disable go test -count=1 -timeout 45m -run '^TestControlPlaneHotPathProfile$' ."
	...
	report := hotPathReport{
		...
		Invocation: hotPathInvocation{
			...
			ExclusionProof:         "TestControlPlaneHotPathProfile skips unless MERC_CONTROL_PLANE_PROFILE=1; listed in ops/scripts/allowed-test-skips.txt; make test / make ci never set the env var",
			Command:                command,
			Fleet:                  fleetN,
			Samples:                samples,
			Trials:                 trials,
		...
		Honesty: hotPathHonesty{
			...
			Guards: []string{
				...
				"MEASURED numbers come from this process; DERIVED frequencies use src/agent/src/vllm.rs HEARTBEAT_INTERVAL=15s; PROJECTED payoff is MEASURED p50 × DERIVED frequency",
