...
func TestReleaseImageShipsEveryFileTheRouterServes(t *testing.T) {
	...
	must(t, err)
	shipsWholeTree := strings.Contains(image, "COPY clients/web/ /web/")
	for _, page := range []string{
		"index.html", "buyer.html", "admin.html", "prices.html", "supplier.html",
	} {
		if _, err := os.Stat(filepath.Join("..", "..", "clients", "web", page)); err != nil {
			continue // not a page this repository has
		}
		if shipsWholeTree || strings.Contains(image, "clients/web/"+page) {
			continue
		}
		t.Errorf("clients/web/%s exists and the image does not ship it", page)
	}
	// The two routes that serve from directories rather than named files.
	for route, dir := range map[string]string{
		"GET /assets/site/{path...}":    "clients/web/assets/site",
		"GET /.well-known/security.txt": "clients/web/.well-known",
	} {
