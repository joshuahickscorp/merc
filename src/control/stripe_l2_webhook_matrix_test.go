...
func TestL2StripeWebhookMatrixRequiresDashboardSecrets(t *testing.T) {
	if os.Getenv(allowSkippingDBTestsEnv) == "1" &&
		(!strings.HasPrefix(strings.TrimSpace(os.Getenv("STRIPE_WEBHOOK_SECRET")), "whsec_") ||
			!strings.HasPrefix(strings.TrimSpace(os.Getenv("MERC_CONNECT_WEBHOOK_SECRET")), "whsec_")) {
		t.Skipf("dashboard webhook secrets are unavailable and %s=1", allowSkippingDBTestsEnv)
	}
