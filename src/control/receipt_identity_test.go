...
func testRepoRoot(t *testing.T) string {
	t.Helper()
	// src/control/ tests run with cwd = src/control/; repo root is two parents up.
	root := filepath.Join("..", "..")
	if _, err := os.Stat(filepath.Join(root, ".git")); err != nil {
		// Worktrees use .git as a file; also accept that.
		if _, err2 := os.Stat(filepath.Join(root, "src", "control")); err2 != nil {
...
func completeIdentity(t *testing.T, root string) (ReceiptIdentity, string) {
	...
	bin := filepath.Join(root, "src", "control", "receipt_identity.go")
