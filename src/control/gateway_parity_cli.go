...
// runGatewayParityCLI is the single live driver. ops/scripts/gateway-parity-v2.go
...
func runGatewayParityCLI(args []string) int {
	...
	notes := []string{
		"Authoritative v2 harness (gate v3 interval decisions). ops/scripts/gateway-parity.py is INVALIDATED.",
...
func runGatewayParityMatrixLive(
	...
	notes := []string{
		...
		"ops/scripts/gateway-parity.py is INVALIDATED.",
		fmt.Sprintf("merc_base_url=%s direct_base_url=%s", mercURL, directURL),
		selection.Rationale,
	}
