...
func directDependencies(root string) dependencyCensus {
	d := dependencyCensus{GoDirect: goDirect(filepath.Join(root, "src", "control", "go.mod")), CargoDirect: cargoDirect(filepath.Join(root, "src", "agent", "Cargo.toml"))}
...
func routeCount(root string) int {
	return countNeedle(filepath.Join(root, "src", "control", "api.go"), "mux.Handle")
}
func tableCount(root string) int {
	return countNeedle(filepath.Join(root, "src", "control", "schema.sql"), "CREATE TABLE IF NOT EXISTS")
}
