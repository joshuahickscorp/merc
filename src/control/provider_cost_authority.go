...
// settlement. Rates share authority with ops/scripts/runpod-spend-guard.py: every
...
//   - nvidia_80gb: $1.19/hr from ops/scripts/runpod-spend-guard.py --self-test is
...
var providerRatesByHWClass = map[string]governedProviderRate{
	...
	"nvidia_80gb": {
		...
		Provenance: "UNBOUND_REFERENCE: $1.19/hr is an example input in " +
			"ops/scripts/runpod-spend-guard.py self-tests, not a bound provider invoice or published price authority",
	},
}
	...
	if !ok {
		return unknownCost(
			"cloud-backed cell " + cellID + " has no resolvable governed provider rate " +
				...
				"bound to the same authority ops/scripts/runpod-spend-guard.py uses")
	}
