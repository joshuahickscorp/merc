...
// src/agent/RAG corpus, plus tool/schema identity-cache avoidance and an
...
// agentRAGRequest is one synthetic src/agent/RAG turn.
//
...
func surveyEngineSignals(t *testing.T) engineSignalSurvey {
	...
	return engineSignalSurvey{
		...
		MercObservationPath: "src/control/prefix_routing.go CorrectPrefixBeliefFromObservation; " +
...
func TestPrefixKVHitRateAgentRAGProductionClaim(t *testing.T) {
	...
	blunt := fmt.Sprintf(
		"Production claim-path belief hit rate on the conservative src/agent/RAG corpus is "+
			...
			"Prefix locality is a real second lever for src/agent/RAG *routing concentration* when the "+
	...
	art := prefixKVArtifact{
		...
		Method: "conservative src/agent/RAG corpus through production ClaimTasksTx " +
	...
	art.CanProve = []string{
		"belief hit rate through production ClaimTasksTx on an src/agent/RAG corpus",
	...
	art.Supersedes.Reason = "Prior artifact ranked candidates in pure Go against a simulated " +
		...
		"This receipt measures belief hit rate on the production claim path with an src/agent/RAG " +
	...
	if os.Getenv(prefixKVHitRateEnv) == "1" {
		...
		id, bin, err := DefaultBoundIdentity("../..",
			"src/control/prefix_kv_hitrate_measure_test.go",
