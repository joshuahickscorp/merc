...
func l12AgentBinary(t *testing.T) string {
	...
	t.Fatal("no merc-agent with emit-infer-artifact; build src/agent/ after adding the command")
	return ""
}
...
func TestL12QuoteRefusalChainFromSource(t *testing.T) {
	...
	l12WriteReceipt(t, "quote-refusal-chain", map[string]any{
		...
		"plane":  "local-source",
		"chain": []string{
			`POST /v1/quote → Server.handleQuote (src/control/quote.go)`,
			...
			`normalizeWorkloadRequest(sub) → normalizeAndValidateJobSubmit (src/control/job_submit_validate.go)`,
			`normalizeAdvertisedRuntimeModelRef → validateAdvertisedRuntimeJobModel (src/control/runtime_matrix.go)`,
			`advertisedRuntimeJobModel iterates advertisedRuntimeCapabilities()`,
			`advertisedRuntimeCapabilities() = currentActivation().advertised (src/control/activation_policy.go)`,
			`cellRoutable requires EffectiveLifecycle == ACTIVE AND cell.Routable`,
			`authorityCell.Routable requires CANARY/ACTIVE plus cellAuthorityBindable (src/control/runtime_authority.go)`,
			`cellAuthorityBindable requires binding_status=BOUND, 16-hex engine_build_hash, exact Apple hardware_identity (src/control/cell_authority_binding.go)`,
