...
func projectProviderCostTerm(
	...
	if !cloud {
		...
		term.Source = "src/control/provider_cost_authority.go"
...
func projectEnergyPartialTerm(
	...
	term.NonMoney = joules
	term.NonMoneyUnit = "joules_per_unit"
	term.Source = entry.Provenance() + " + src/control/pricing.go:defaultElectricityUSDPerKWh"
	term.Basis = fmt.Sprintf(
...
func cellEconomicsEvidenceAuthority(
	...
	auth := []string{
		"src/control/runtime_cell_cost.go#MeasuredSupplierLiabilityProxy",
