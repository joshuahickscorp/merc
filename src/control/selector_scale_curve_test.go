...
//	  bash -c 'cd src/control && go test -count=1 -run '^TestSelectorScaleCurve$' -timeout 6h .'
//
...
const (
	...
	// Production supplier cadence: src/agent/src/vllm.rs HEARTBEAT_INTERVAL is 15s
...
func TestSelectorScaleCurve(t *testing.T) {
	...
	report := selectorScaleReport{
		...
		Invocation: selectorScaleInvocation{
			...
			Command:                "bash ops/scripts/with-isolated-test-db.sh bash -c 'cd src/control && go test -count=1 -run ^TestSelectorScaleCurve$ -timeout 6h .'",
			ExcludedFromNormalGate: true,
			ExclusionProof:         "TestSelectorScaleCurve skips unless MERC_SELECTOR_SCALE_CURVE=1; listed in ops/scripts/allowed-test-skips.txt; make test / make ci never set the env var",
		},
...
func measureSelectorScaleSamples(
	...
	// (src/agent/src/vllm.rs HEARTBEAT_INTERVAL) against the 45s liveness window in
...
func writeSelectorScaleEvidence(report selectorScaleReport) error {
	// src/control/ tests run with cwd=src/control/; evidence lives at repo root.
	//
