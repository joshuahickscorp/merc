...
//
// Writes evidence/perf/render-verify-pipeline.json when run from src/control/.
//
...
func TestRenderVerifyPipelineBench(t *testing.T) {
	...
	report := map[string]any{
		...
		"invocation": map[string]any{
			...
			"exclusion_proof":            "TestRenderVerifyPipelineBench skips unless MERC_RENDER_VERIFY_PIPELINE=1; listed in ops/scripts/allowed-test-skips.txt; make test / make ci never set the env var",
			"command":                    "cd src/control && MERC_RENDER_VERIFY_PIPELINE=1 go test -count=1 -run '^TestRenderVerifyPipelineBench$' -timeout 90m .",
			"blender_bin":                blender,
