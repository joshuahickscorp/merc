...
//
// Writes evidence/perf/liveness-index-bench.json when run from src/control/.
...
func TestLiveDeviceIndexBench(t *testing.T) {
	...
	report := livenessIndexBenchReport{
		...
		Invocation: livenessIndexBenchInvocation{
			...
			ExclusionProof:         "TestLiveDeviceIndexBench skips unless MERC_LIVENESS_INDEX_BENCH=1; listed in ops/scripts/allowed-test-skips.txt; make test / make ci never set the env var",
