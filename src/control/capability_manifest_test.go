...
// than producing a wrong answer. src/agent/src/runtime_authority.rs pins the same
...
func TestCapabilityMatrixDigestIsPinnedAcrossBothImplementations(t *testing.T) {
	if generatedRuntimeMatrixSHA256 != pinnedCapabilityMatrixDigest {
		t.Fatalf("capability matrix digest moved\n  got  %s\n  want %s\n"+
			...
			"src/agent/src/runtime_authority.rs, and rebuild the agent.",
			generatedRuntimeMatrixSHA256, pinnedCapabilityMatrixDigest)
	}
}

func TestCapabilitySourceIdentityLayoutAliasPreservesProfileDigest(t *testing.T) {
	document := deepCopyAuthority(t)
	profile := document.Runtimes[0]
	canonical := profile.SourceIdentity
	profile.SourceIdentity = "control/runtime-authority.json"
	legacyDigest, err := profile.CapabilityDigest(document.authorityModels())
	must(t, err)
	profile.SourceIdentity = canonical
	canonicalDigest, err := profile.CapabilityDigest(document.authorityModels())
	must(t, err)
	if canonicalDigest != legacyDigest {
		t.Fatalf("authority layout alias changed profile digest: canonical=%s legacy=%s",
			canonicalDigest, legacyDigest)
	}
}
