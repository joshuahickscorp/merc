...
func TestPublicPathExactReuse128To1WritesSealedReceipt(t *testing.T) {
	...
	out := map[string]any{
		...
		"path": map[string]any{
			...
			"not_the_path":    "src/control/exact_reuse_batch.go (batchExactReuseEnabled=false)",
		...
		"supersedes": map[string]any{
			...
			"reasons": []string{
				"batchExactReuseEnabled is compile-time false in src/control/exact_reuse_batch.go:21, so the batch path cannot settle a live cache hit",
				"SubmitExactReuseBatchJob requires workload batch_infer while the old artifact described embed jobs",
				"the historical batch identity build omitted TenantScope, which RequestIdentity.Compute refuses (src/control/exact_reuse.go:110-112), so the key was empty even if the flag were flipped",
			},
		},
	}

	id, bin, err := DefaultBoundIdentity("../..",
		"src/control/public_path_reuse_proof_test.go#TestPublicPathExactReuse128To1WritesSealedReceipt",
