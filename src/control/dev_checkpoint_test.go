...
func TestCheckpointReadsTheMutationScriptsOwnLock(t *testing.T) {
	...
	script, err := os.ReadFile(filepath.Join(root, "ops", "scripts", "mutation-test.sh"))
	must(t, err)
...
func TestCheckpointUsesTheIsolatedParallelMutationRunner(t *testing.T) {
	...
	checkpoint, err := os.ReadFile(filepath.Join(root, "src", "control", "dev_checkpoint.go"))
	must(t, err)
	if !strings.Contains(string(checkpoint), `"bash", "ops/scripts/mutation-test-parallel.sh"`) {
		t.Fatal("checkpoint no longer runs the isolated parallel mutation campaign")
	}
	parallel, err := os.ReadFile(filepath.Join(root, "ops", "scripts", "mutation-test-parallel.sh"))
	must(t, err)
	for _, fragment := range []string{
