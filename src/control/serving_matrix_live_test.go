...
func TestLiveServingMatrixLlamaCppMetal(t *testing.T) {
	...
	notes := []string{
		...
		"finding: ops/scripts/gateway-parity.py evaluate_budget only inspects concurrency=1; this harness evaluates every claimed level",
	}
	...
	if os.Getenv("MERC_SERVING_MATRIX_PERF") == "1" {
		...
		id, bin, err := DefaultBoundIdentity("../..", "src/control/serving_matrix_live_test.go",
...
func TestEngineTournamentIncomparableArmsAndHostScope(t *testing.T) {
	...
	if os.Getenv("MERC_ENGINE_TOURNAMENT_PERF") == "1" {
		...
		id, bin, err := DefaultBoundIdentity("../..",
			"src/control/serving_matrix_live_test.go#TestEngineTournamentIncomparableArmsAndHostScope",
